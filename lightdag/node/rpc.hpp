...
{
public:
	rpc (boost::asio::io_service &, lightdag::node &, lightdag::rpc_config const &);
	void start ();
	virtual void accept ();
	void stop ();
	void observer_action (lightdag::account const &);
	boost::asio::ip::tcp::acceptor acceptor;
	std::mutex mutex;
	std::unordered_map<lightdag::account, std::shared_ptr<lightdag::payment_observer>> payment_observers;
	lightdag::rpc_config config;
	lightdag::node & node;
	bool on;
	static uint16_t const rpc_port = lightdag::lightdag_network == lightdag::lightdag_networks::lightdag_live_network ? 7076 : 55000;
};
class rpc_connection : public std::enable_shared_from_this<lightdag::rpc_connection>
{
public:
	rpc_connection (lightdag::node &, lightdag::rpc &);
...
class payment_observer : public std::enable_shared_from_this<lightdag::payment_observer>
{
public:
	payment_observer (std::function<void(boost::property_tree::ptree const &)> const &, lightdag::rpc &, lightdag::account const &, lightdag::amount const &);
	~payment_observer ();
	void start (uint64_t);
	void observe ();
	void timeout ();
	void complete (lightdag::payment_status);
	std::mutex mutex;
...
class rpc_handler : public std::enable_shared_from_this<lightdag::rpc_handler>
{
public:
	rpc_handler (lightdag::node &, lightdag::rpc &, std::string const &, std::function<void(boost::property_tree::ptree const &)> const &);
	...
	void key_create ();
	void key_expand ();
	void klightdag_to_raw ();
	void klightdag_from_raw ();
	void ledger ();
	void mlightdag_to_raw ();
	void mlightdag_from_raw ();
	...
	void pending_exists ();
	void process ();
	void lightdag_to_raw ();
	void lightdag_from_raw ();
	void receive ();
...
std::unique_ptr<lightdag::rpc> get_rpc (boost::asio::io_service & service_a, lightdag::node & node_a, lightdag::rpc_config const & config_a);
}
