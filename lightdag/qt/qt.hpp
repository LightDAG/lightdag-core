...
public:
	...
	QPushButton * back;
	lightdag_qt::wallet & wallet;
};
class advanced_actions
{
public:
	advanced_actions (lightdag_qt::wallet &);
	QWidget * window;
	QVBoxLayout * layout;
...
{
public:
	block_entry (lightdag_qt::wallet &);
	QWidget * window;
	QVBoxLayout * layout;
...
{
public:
	block_creation (lightdag_qt::wallet &);
	void deactivate_all ();
	void activate_send ();
...
{
public:
	self_pane (lightdag_qt::wallet &, lightdag::account const &);
...
{
public:
	history (lightdag::ledger &, lightdag::account const &, lightdag_qt::wallet &);
	void refresh ();
	QWidget * window;
	QVBoxLayout * layout;
	QStandardItemModel * model;
	...
	lightdag::account const & account;
	lightdag_qt::wallet & wallet;
};
class block_viewer
{
public:
	block_viewer (lightdag_qt::wallet &);
	void rebroadcast_action (lightdag::uint256_union const &);
	QWidget * window;
	QVBoxLayout * layout;
	QLabel * hash_label;
	QLineEdit * hash;
...
{
public:
	account_viewer (lightdag_qt::wallet &);
	QWidget * window;
	QVBoxLayout * layout;
	QLabel * account_label;
...
{
public:
	status (lightdag_qt::wallet &);
	void erase (lightdag_qt::status_types);
	void insert (lightdag_qt::status_types);
	void set_text ();
	std::string text ();
	std::string color ();
	std::set<lightdag_qt::status_types> active;
	lightdag_qt::wallet & wallet;
};
class wallet : public std::enable_shared_from_this<lightdag_qt::wallet>
{
public:
	wallet (QApplication &, lightdag_qt::eventloop_processor &, lightdag::node &, std::shared_ptr<lightdag::wallet>, lightdag::account &);
	void start ();
	void refresh ();
	void update_connected ();
	void empty_password ();
	void change_rendering_ratio (lightdag::uint128_t const &);
	std::string format_balance (lightdag::uint128_t const &) const;
	lightdag::uint128_t rendering_ratio;
	lightdag::node & node;
	std::shared_ptr<lightdag::wallet> wallet_m;
	lightdag::account & account;
	lightdag_qt::eventloop_processor & processor;
	lightdag_qt::history history;
	lightdag_qt::accounts accounts;
	lightdag_qt::self_pane self;
	lightdag_qt::settings settings;
	lightdag_qt::advanced_actions advanced;
	lightdag_qt::block_creation block_creation;
	lightdag_qt::block_entry block_entry;
	lightdag_qt::block_viewer block_viewer;
	lightdag_qt::account_viewer account_viewer;
	lightdag_qt::import import;
	...
	lightdag_qt::status active_status;
	void pop_main_stack ();
