...
if [[ "$OSTYPE" == "linux-gnu" ]]; then
    ...
    DISTRO_CFG="-DLIGHTDAG_DISTRO_NAME=${distro}"
    ...
    CPACK_TYPE="TBZ2"
    DISTRO_CFG="-DLIGHTDAG_DISTRO_NAME='freebsd'"
else
    CPACK_TYPE="TBZ2"
fi

if [[ ${SIMD} -eq 1 ]]; then
    SIMD_CFG="-DLIGHTDAG_SIMD_OPTIMIZATIONS=ON"
    CRYPTOPP_CFG=""
...
if [[ ${ASAN_INT} -eq 1 ]]; then
    SANITIZERS="-DLIGHTDAG_ASAN_INT=ON"
elif [[ ${ASAN} -eq 1 ]]; then
    SANITIZERS="-DLIGHTDAG_ASAN=ON"
elif [[ ${TSAN} -eq 1 ]]; then
    SANITIZERS="-DLIGHTDAG_TSAN=ON"
else
    SANITIZERS=""
fi
