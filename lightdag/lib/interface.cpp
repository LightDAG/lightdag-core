...
extern "C" {
void ldg_uint128_to_dec (ldg_uint128 source, char * destination)
{
	auto const & number (*reinterpret_cast<lightdag::uint128_union *> (source));
	strncpy (destination, number.to_string_dec ().c_str (), 32);
}

void ldg_uint256_to_string (ldg_uint256 source, char * destination)
{
	auto const & number (*reinterpret_cast<lightdag::uint256_union *> (source));
...
void ldg_uint512_to_string (ldg_uint512 source, char * destination)
{
	auto const & number (*reinterpret_cast<lightdag::uint512_union *> (source));
	strncpy (destination, number.to_string ().c_str (), 128);
}

int ldg_uint128_from_dec (const char * source, ldg_uint128 destination)
{
	auto & number (*reinterpret_cast<lightdag::uint128_union *> (destination));
...
{
	auto & number (*reinterpret_cast<lightdag::uint256_union *> (destination));
...
{
	auto & number (*reinterpret_cast<lightdag::uint512_union *> (destination));
	auto error (number.decode_hex (source));
	return error ? 1 : 0;
}

int ldg_valid_address (const char * account_a)
{
...
void ldg_generate_random (ldg_uint256 seed)
{
	auto & number (*reinterpret_cast<lightdag::uint256_union *> (seed));
	lightdag::random_pool.GenerateBlock (number.bytes.data (), number.bytes.size ());
}

void ldg_seed_key (ldg_uint256 seed, int index, ldg_uint256 destination)
{
	auto & seed_l (*reinterpret_cast<lightdag::uint256_union *> (seed));
	auto & destination_l (*reinterpret_cast<lightdag::uint256_union *> (destination));
	lightdag::deterministic_key (seed_l, index, destination_l);
}

void ldg_key_account (const ldg_uint256 key, ldg_uint256 pub)
{
...
char * ldg_sign_transaction (const char * transaction, const ldg_uint256 private_key)
{
	...
	try
	{
		...
		if (block != nullptr)
		{
			...
			prv.data = *reinterpret_cast<lightdag::uint256_union *> (private_key);
			block->signature_set (lightdag::sign_message (prv, pub, block->hash ()));
...
char * ldg_work_transaction (const char * transaction)
{
	...
	try
	{
		...
		if (block != nullptr)
		{
			lightdag::work_pool pool (std::thread::hardware_concurrency ());
...
{
	lightdag::random_pool.GenerateBlock (reinterpret_cast<uint8_t *> (out), outlen);
}
