...
bool lightdag_qt::eventloop_processor::event (QEvent * event_a)
{
	assert (dynamic_cast<lightdag_qt::eventloop_event *> (event_a) != nullptr);
	static_cast<lightdag_qt::eventloop_event *> (event_a)->action ();
	return true;
}

lightdag_qt::eventloop_event::eventloop_event (std::function<void()> const & action_a) :
QEvent (QEvent::Type::User),
action (action_a)
{
}

lightdag_qt::self_pane::self_pane (lightdag_qt::wallet & wallet_a, lightdag::account const & account_a) :
window (new QWidget),
layout (new QVBoxLayout),
self_layout (new QHBoxLayout),
self_window (new QWidget),
your_account_label (new QLabel ("Your LightDAG account:")),
...
wallet (wallet_a)
{
	...
	version = new QLabel (boost::str (boost::format ("Version %1%.%2%") % LIGHTDAG_VERSION_MAJOR % LIGHTDAG_VERSION_MINOR).c_str ());
...
void lightdag_qt::self_pane::refresh_balance ()
{
...
lightdag_qt::accounts::accounts (lightdag_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
...
wallet (wallet_a)
{
	...
	QObject::connect (create_account, &QPushButton::released, [this]() {
		lightdag::transaction transaction (this->wallet.wallet_m->store.environment, nullptr, true);
	...
	QObject::connect (backup_seed, &QPushButton::released, [this]() {
		...
		lightdag::transaction transaction (this->wallet.wallet_m->store.environment, nullptr, false);
		if (this->wallet.wallet_m->store.valid_password (transaction))
		{
...
void lightdag_qt::accounts::refresh_wallet_balance ()
{
	lightdag::transaction transaction (this->wallet.wallet_m->store.environment, nullptr, false);
...
void lightdag_qt::accounts::refresh ()
{
	model->removeRows (0, model->rowCount ());
	lightdag::transaction transaction (wallet.wallet_m->store.environment, nullptr, false);
	QBrush brush;
...
lightdag_qt::import::import (lightdag_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
...
lightdag_qt::history::history (lightdag::ledger & ledger_a, lightdag::account const & account_a, lightdag_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
model (new QStandardItemModel),
view (new QTableView),
...
namespace
{
class short_text_visitor : public lightdag::block_visitor
{
public:
	short_text_visitor (MDB_txn * transaction_a, lightdag::ledger & ledger_a) :
	...
	void open_block (lightdag::open_block const & block_a)
	{
		...
		{
			account = lightdag::genesis_account;
			amount = lightdag::genesis_amount;
		}
	}
	...
	std::string type;
	lightdag::uint128_t amount;
	lightdag::account account;
};
}

void lightdag_qt::history::refresh ()
{
	lightdag::transaction transaction (ledger.store.environment, nullptr, false);
	model->removeRows (0, model->rowCount ());
...
lightdag_qt::block_viewer::block_viewer (lightdag_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
...
wallet (wallet_a)
{
	...
	QObject::connect (retrieve, &QPushButton::released, [this]() {
		lightdag::block_hash hash_l;
		if (!hash_l.decode_hex (hash->text ().toStdString ()))
		{
			lightdag::transaction transaction (this->wallet.node.store.environment, nullptr, false);
...
void lightdag_qt::block_viewer::rebroadcast_action (lightdag::uint256_union const & hash_a)
{
	auto done (true);
	lightdag::transaction transaction (wallet.node.ledger.store.environment, nullptr, false);
...
lightdag_qt::account_viewer::account_viewer (lightdag_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
...
wallet (wallet_a)
{
	...
	QObject::connect (refresh, &QPushButton::released, [this]() {
		...
		if (!account.decode_account (account_line->text ().toStdString ()))
		{
			...
			auto final_text (std::string ("Balance (LDG): ") + wallet.format_balance (balance.first));
			if (!balance.second.is_zero ())
			{
				final_text += "\nPending: " + wallet.format_balance (balance.second);
			}
...
lightdag_qt::status::status (lightdag_qt::wallet & wallet_a) :
wallet (wallet_a)
{
	wallet.status->setToolTip ("Wallet status, block count (blocks downloaded)");
	active.insert (lightdag_qt::status_types::nominal);
	set_text ();
}

void lightdag_qt::status::erase (lightdag_qt::status_types status_a)
{
	assert (status_a != lightdag_qt::status_types::nominal);
	auto erased (active.erase (status_a));
	(void)erased;
	set_text ();
}

void lightdag_qt::status::insert (lightdag_qt::status_types status_a)
{
	assert (status_a != lightdag_qt::status_types::nominal);
...
std::string lightdag_qt::status::text ()
{
	...
	{
		lightdag::transaction transaction (wallet.wallet_m->node.store.environment, nullptr, false);
	...
	switch (*active.begin ())
	{
		case lightdag_qt::status_types::disconnected:
			result = "Status: Disconnected";
			break;
		case lightdag_qt::status_types::working:
			result = "Status: Generating proof of work";
			break;
		case lightdag_qt::status_types::synchronizing:
			result = "Status: Synchronizing";
			break;
		case lightdag_qt::status_types::locked:
			result = "Status: Wallet locked";
			break;
		case lightdag_qt::status_types::vulnerable:
			result = "Status: Wallet password empty";
			break;
		case lightdag_qt::status_types::active:
			result = "Status: Wallet active";
			break;
		case lightdag_qt::status_types::nominal:
...
std::string lightdag_qt::status::color ()
{
	assert (!active.empty ());
	std::string result;
	switch (*active.begin ())
...
lightdag_qt::wallet::wallet (QApplication & application_a, lightdag_qt::eventloop_processor & processor_a, lightdag::node & node_a, std::shared_ptr<lightdag::wallet> wallet_a, lightdag::account & account_a) :
rendering_ratio (lightdag::Mldg_ratio),
node (node_a),
wallet_m (wallet_a),
account (account_a),
...
active_status (*this)
{
	...
	send_account->setPlaceholderText (lightdag::zero_key.pub.to_account ().c_str ());
...
void lightdag_qt::wallet::start ()
{
	std::weak_ptr<lightdag_qt::wallet> this_w (shared_from_this ());
	...
	QObject::connect (send_blocks_send, &QPushButton::released, [this_w]() {
		if (auto this_l = this_w.lock ())
		{
			...
			{
				lightdag::uint128_t actual (amount.number () * this_l->rendering_ratio);
				if (actual / this_l->rendering_ratio == amount.number ())
				{
					...
					if (!parse_error)
					{
						...
						if (actual <= balance)
						{
							lightdag::transaction transaction (this_l->wallet_m->store.environment, nullptr, false);
							if (this_l->wallet_m->store.valid_password (transaction))
							{
								...
								this_l->node.background ([this_w, account_l, actual]() {
									if (auto this_l = this_w.lock ())
									{
										this_l->wallet_m->send_async (this_l->account, account_l, actual, [this_w](std::shared_ptr<lightdag::block> block_a) {
											if (auto this_l = this_w.lock ())
											{
	...
	node.observers.blocks.add ([this_w](std::shared_ptr<lightdag::block> block_a, lightdag::process_return const & result_a) {
		if (auto this_l = this_w.lock ())
		{
	...
	node.observers.account_balance.add ([this_w](lightdag::account const & account_a, bool is_pending) {
		if (auto this_l = this_w.lock ())
		{
		...
		if (auto this_l = this_w.lock ())
		{
			this_l->application.postEvent (&this_l->processor, new eventloop_event ([this_w, active_a]() {
				if (auto this_l = this_w.lock ())
				{
					if (active_a)
					{
						this_l->active_status.insert (lightdag_qt::status_types::active);
					}
					else
					{
						this_l->active_status.erase (lightdag_qt::status_types::active);
					}
				}
			}));
		}
	});
	node.observers.endpoint.add ([this_w](lightdag::endpoint const &) {
		if (auto this_l = this_w.lock ())
		{
			this_l->application.postEvent (&this_l->processor, new eventloop_event ([this_w]() {
				if (auto this_l = this_w.lock ())
		...
		if (auto this_l = this_w.lock ())
		{
			this_l->application.postEvent (&this_l->processor, new eventloop_event ([this_w, active_a]() {
				if (auto this_l = this_w.lock ())
				{
					if (active_a)
					{
						this_l->active_status.insert (lightdag_qt::status_types::synchronizing);
					}
					else
					{
						this_l->active_status.erase (lightdag_qt::status_types::synchronizing);
					}
				}
			}));
		}
		...
		if (auto this_l = this_w.lock ())
		{
			this_l->application.postEvent (&this_l->processor, new eventloop_event ([this_w, working]() {
				if (auto this_l = this_w.lock ())
				{
					if (working)
					{
						this_l->active_status.insert (lightdag_qt::status_types::working);
					}
					else
					{
						this_l->active_status.erase (lightdag_qt::status_types::working);
					}
				}
			}));
		}
...
void lightdag_qt::wallet::refresh ()
{
	{
		lightdag::transaction transaction (wallet_m->store.environment, nullptr, false);
...
void lightdag_qt::wallet::update_connected ()
{
	if (node.peers.empty ())
	{
		active_status.insert (lightdag_qt::status_types::disconnected);
	}
	else
	{
		active_status.erase (lightdag_qt::status_types::disconnected);
	}
}

void lightdag_qt::wallet::empty_password ()
{
...
void lightdag_qt::wallet::change_rendering_ratio (lightdag::uint128_t const & rendering_ratio_a)
{
...
std::string lightdag_qt::wallet::format_balance (lightdag::uint128_t const & balance) const
{
	auto balance_str = lightdag::amount (balance).format_balance (rendering_ratio, 0, false);
	auto unit = std::string ("LDG");
	if (rendering_ratio == lightdag::kldg_ratio)
	{
		unit = std::string ("kldg");
	}
	else if (rendering_ratio == lightdag::ldg_ratio)
	{
		unit = std::string ("ldg");
	}
...
void lightdag_qt::wallet::push_main_stack (QWidget * widget_a)
{
	main_stack->addWidget (widget_a);
	main_stack->setCurrentIndex (main_stack->count () - 1);
}

void lightdag_qt::wallet::pop_main_stack ()
{
	main_stack->removeWidget (main_stack->currentWidget ());
}

lightdag_qt::settings::settings (lightdag_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
password (new QLineEdit),
...
wallet (wallet_a)
{
	...
	new_representative->setPlaceholderText (lightdag::zero_key.pub.to_account ().c_str ());
	...
	QObject::connect (change_rep, &QPushButton::released, [this]() {
		lightdag::account representative_l;
		if (!representative_l.decode_account (new_representative->text ().toStdString ()))
		{
			...
			if (this->wallet.wallet_m->store.valid_password (transaction))
			{
				...
				{
					lightdag::transaction transaction_l (this->wallet.wallet_m->store.environment, nullptr, true);
...
void lightdag_qt::settings::refresh_representative ()
{
	lightdag::transaction transaction (this->wallet.wallet_m->node.store.environment, nullptr, false);
...
void lightdag_qt::settings::activate ()
{
	this->wallet.push_main_stack (window);
}

void lightdag_qt::settings::update_locked (bool invalid, bool vulnerable)
{
	if (invalid)
	{
		this->wallet.active_status.insert (lightdag_qt::status_types::locked);
	}
	else
	{
		this->wallet.active_status.erase (lightdag_qt::status_types::locked);
	}
	if (vulnerable)
	{
		this->wallet.active_status.insert (lightdag_qt::status_types::vulnerable);
	}
	else
	{
		this->wallet.active_status.erase (lightdag_qt::status_types::vulnerable);
	}
}

lightdag_qt::advanced_actions::advanced_actions (lightdag_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
...
mldg (new QRadioButton ("Mldg")),
kldg (new QRadioButton ("kldg")),
ldg (new QRadioButton ("ldg")),
back (new QPushButton ("Back")),
...
wallet (wallet_a)
{
	ratio_group->addButton (mldg);
	ratio_group->addButton (kldg);
	ratio_group->addButton (ldg);
	ratio_group->setId (mldg, 0);
	ratio_group->setId (kldg, 1);
	ratio_group->setId (ldg, 2);
	scale_layout->addWidget (scale_label);
	scale_layout->addWidget (mldg);
	scale_layout->addWidget (kldg);
	scale_layout->addWidget (ldg);
	...
	QObject::connect (mldg, &QRadioButton::toggled, [this]() {
		if (mldg->isChecked ())
		{
			this->wallet.change_rendering_ratio (lightdag::Mldg_ratio);
		}
	});
	QObject::connect (kldg, &QRadioButton::toggled, [this]() {
		if (kldg->isChecked ())
		{
			this->wallet.change_rendering_ratio (lightdag::kldg_ratio);
		}
	});
	QObject::connect (ldg, &QRadioButton::toggled, [this]() {
		if (ldg->isChecked ())
		{
			this->wallet.change_rendering_ratio (lightdag::ldg_ratio);
		}
	});
	mldg->click ();
	...
	QObject::connect (peers_bootstrap, &QPushButton::released, [this]() {
		...
		auto error (lightdag::parse_endpoint (bootstrap_line->text ().toStdString (), endpoint));
		if (!error)
		{
...
void lightdag_qt::advanced_actions::refresh_peers ()
{
...
void lightdag_qt::advanced_actions::refresh_ledger ()
{
	...
	{
		QList<QStandardItem *> items;
		items.push_back (new QStandardItem (QString (lightdag::block_hash (i->first.uint256 ()).to_account ().c_str ())));
		lightdag::account_info info (i->second);
		std::string balance;
		lightdag::amount (info.balance.number () / wallet.rendering_ratio).encode_dec (balance);
...
lightdag_qt::block_entry::block_entry (lightdag_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
block (new QPlainTextEdit),
status (new QLabel),
...
wallet (wallet_a)
{
	...
	QObject::connect (process, &QPushButton::released, [this]() {
		...
		try
		{
			...
			auto block_l (lightdag::deserialize_block_json (tree));
			if (block_l != nullptr)
			{
...
lightdag_qt::block_creation::block_creation (lightdag_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
...
void lightdag_qt::block_creation::deactivate_all ()
{
	account_label->hide ();
	account->hide ();
	source_label->hide ();
...
void lightdag_qt::block_creation::activate_send ()
{
	account_label->show ();
	account->show ();
	amount_label->show ();
...
void lightdag_qt::block_creation::activate_receive ()
{
	source_label->show ();
	source->show ();
}

void lightdag_qt::block_creation::activate_open ()
{
	source_label->show ();
	source->show ();
...
void lightdag_qt::block_creation::activate_change ()
{
	account_label->show ();
	account->show ();
	representative_label->show ();
			...
			if (!error)
			{
				...
				if (!wallet.wallet_m->store.fetch (transaction, account_l, key))
				{
					...
					if (amount_l.number () <= balance)
					{
						...
						assert (!error);
						lightdag::send_block send (info.head, destination_l, balance - amount_l.number (), key, account_l, wallet.wallet_m->work_fetch (transaction, account_l, info.head));
						std::string block_l;
...
void lightdag_qt::block_creation::create_receive ()
{
	lightdag::block_hash source_l;
	...
	if (!error)
	{
		...
		if (block_l != nullptr)
		{
			...
			{
				lightdag::pending_key pending_key (destination, source_l);
				...
				if (!wallet.node.store.pending_get (transaction, pending_key, pending))
				{
					...
					if (!error)
					{
						...
						if (!error)
						{
							lightdag::receive_block receive (info.head, source_l, key, pending_key.account, wallet.wallet_m->work_fetch (transaction, pending_key.account, info.head));
							std::string block_l;
...
void lightdag_qt::block_creation::create_change ()
{
	lightdag::account account_l;
	auto error (account_l.decode_account (account->text ().toStdString ()));
	if (!error)
	{
		...
		if (!error)
		{
			...
			if (!error)
			{
				...
				if (!error)
				{
					lightdag::change_block change (info.head, representative_l, key, account_l, wallet.wallet_m->work_fetch (transaction, account_l, info.head));
					std::string block_l;
...
void lightdag_qt::block_creation::create_open ()
{
	lightdag::block_hash source_l;
	auto error (source_l.decode_hex (source->text ().toStdString ()));
	if (!error)
				...
				if (!destination.is_zero ())
				{
					...
					if (!wallet.node.store.pending_get (transaction, pending_key, pending))
					{
						...
						if (error)
						{
							...
							if (!error)
							{
								lightdag::open_block open (source_l, representative_l, pending_key.account, key, pending_key.account, wallet.wallet_m->work_fetch (transaction, pending_key.account, pending_key.account));
								std::string block_l;
