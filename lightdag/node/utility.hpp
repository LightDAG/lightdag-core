...
{
	bool error (false);
	lightdag::open_or_create (stream_a, path_a.string ());
	if (!stream_a.fail ())
	{
		boost::property_tree::ptree tree;
		try
...
public:
	...
	mdb_val (size_t, void *);
	mdb_val (lightdag::uint128_union const &);
	mdb_val (lightdag::uint256_union const &);
	void * data () const;
	size_t size () const;
	lightdag::uint256_union uint256 () const;
...
{
public:
	transaction (lightdag::mdb_env &, MDB_txn *, bool);
	~transaction ();
