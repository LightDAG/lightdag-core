...
lightdag::block_synchronization::block_synchronization (boost::log::sources::logger_mt & log_a) :
log (log_a)
{
}
...
namespace
{
class add_dependency_visitor : public lightdag::block_visitor
{
public:
	add_dependency_visitor (MDB_txn * transaction_a, lightdag::block_synchronization & sync_a) :
	...
	void add_dependency (lightdag::block_hash const & hash_a)
	{
	...
	MDB_txn * transaction;
	lightdag::block_synchronization & sync;
	bool complete;
};
}

bool lightdag::block_synchronization::add_dependency (MDB_txn * transaction_a, lightdag::block const & block_a)
{
...
void lightdag::block_synchronization::fill_dependencies (MDB_txn * transaction_a)
{
	auto done (false);
	while (!done)
	{
...
lightdag::sync_result lightdag::block_synchronization::synchronize_one (MDB_txn * transaction_a)
{
	...
	lightdag::sync_result result (lightdag::sync_result::success);
	auto hash (blocks.back ());
	blocks.pop_back ();
	auto block (retrieve (transaction_a, hash));
	if (block != nullptr)
...
lightdag::sync_result lightdag::block_synchronization::synchronize (MDB_txn * transaction_a, lightdag::block_hash const & hash_a)
{
	auto result (lightdag::sync_result::success);
	blocks.clear ();
	blocks.push_back (hash_a);
	auto cutoff (std::chrono::steady_clock::now () + lightdag::transaction_timeout);
	while (std::chrono::steady_clock::now () < cutoff && result != lightdag::sync_result::fork && !blocks.empty ())
	{
...
lightdag::push_synchronization::push_synchronization (lightdag::node & node_a, std::function<lightdag::sync_result (MDB_txn *, lightdag::block const &)> const & target_a) :
...
bool lightdag::push_synchronization::synchronized (MDB_txn * transaction_a, lightdag::block_hash const & hash_a)
{
...
std::unique_ptr<lightdag::block> lightdag::push_synchronization::retrieve (MDB_txn * transaction_a, lightdag::block_hash const & hash_a)
{
	return node.store.block_get (transaction_a, hash_a);
}

lightdag::sync_result lightdag::push_synchronization::target (MDB_txn * transaction_a, lightdag::block const & block_a)
{
	return target_m (transaction_a, block_a);
}

lightdag::bootstrap_client::bootstrap_client (std::shared_ptr<lightdag::node> node_a, std::shared_ptr<lightdag::bootstrap_attempt> attempt_a, lightdag::tcp_endpoint const & endpoint_a) :
node (node_a),
attempt (attempt_a),
...
lightdag::bootstrap_client::~bootstrap_client ()
{
	--attempt->connections;
}

double lightdag::bootstrap_client::block_rate () const
{
	auto elapsed = elapsed_seconds ();
	return elapsed > 0.0 ? (double)block_count.load () / elapsed : 0.0;
}

double lightdag::bootstrap_client::elapsed_seconds () const
{
	return std::chrono::duration_cast<std::chrono::duration<double>> (std::chrono::steady_clock::now () - start_time).count ();
}

void lightdag::bootstrap_client::stop (bool force)
{
	pending_stop = true;
	if (force)
	{
...
void lightdag::bootstrap_client::start_timeout ()
{
	timeout.expires_from_now (boost::posix_time::seconds (5));
	std::weak_ptr<lightdag::bootstrap_client> this_w (shared ());
...
void lightdag::bootstrap_client::stop_timeout ()
{
	size_t killed (timeout.cancel ());
	(void)killed;
}

void lightdag::bootstrap_client::run ()
{
...
void lightdag::frontier_req_client::run ()
{
	std::unique_ptr<lightdag::frontier_req> request (new lightdag::frontier_req);
	request->start.clear ();
	...
	{
		lightdag::vectorstream stream (*send_buffer);
...
std::shared_ptr<lightdag::bootstrap_client> lightdag::bootstrap_client::shared ()
{
	return shared_from_this ();
}

lightdag::frontier_req_client::frontier_req_client (std::shared_ptr<lightdag::bootstrap_client> connection_a) :
connection (connection_a),
current (0),
count (0),
next_report (std::chrono::steady_clock::now () + std::chrono::seconds (15))
{
	lightdag::transaction transaction (connection->node->store.environment, nullptr, false);
	next (transaction);
}

lightdag::frontier_req_client::~frontier_req_client ()
{
}

void lightdag::frontier_req_client::receive_frontier ()
{
	auto this_l (shared_from_this ());
	connection->start_timeout ();
	size_t size_l (sizeof (lightdag::uint256_union) + sizeof (lightdag::uint256_union));
...
void lightdag::frontier_req_client::unsynced (MDB_txn * transaction_a, lightdag::block_hash const & ours_a, lightdag::block_hash const & theirs_a)
{
	auto current (ours_a);
...
void lightdag::frontier_req_client::received_frontier (boost::system::error_code const & ec, size_t size_a)
{
	if (!ec)
	{
		assert (size_a == sizeof (lightdag::uint256_union) + sizeof (lightdag::uint256_union));
		lightdag::account account;
		lightdag::bufferstream account_stream (connection->receive_buffer.data (), sizeof (lightdag::uint256_union));
		auto error1 (lightdag::read (account_stream, account));
		assert (!error1);
		lightdag::block_hash latest;
		lightdag::bufferstream latest_stream (connection->receive_buffer.data () + sizeof (lightdag::uint256_union), sizeof (lightdag::uint256_union));
		auto error2 (lightdag::read (latest_stream, latest));
		assert (!error2);
		if (count == 0)
		{
			start_time = std::chrono::steady_clock::now ();
		...
		if (!account.is_zero ())
		{
			while (!current.is_zero () && current < account)
			{
				...
				lightdag::transaction transaction (connection->node->store.environment, nullptr, true);
			...
			if (!current.is_zero ())
			{
				if (account == current)
				{
					...
					else
					{
						...
						{
							connection->attempt->add_pull (lightdag::pull_info (account, latest, info.head));
						}
					}
					next (transaction);
				}
				else
				{
					...
					connection->attempt->add_pull (lightdag::pull_info (account, latest, lightdag::block_hash (0)));
				}
			}
			else
			{
...
void lightdag::frontier_req_client::next (MDB_txn * transaction_a)
{
	auto iterator (connection->node->store.latest_begin (transaction_a, lightdag::uint256_union (current.number () + 1)));
	if (iterator != connection->node->store.latest_end ())
	{
		current = lightdag::account (iterator->first.uint256 ());
		info = lightdag::account_info (iterator->second);
	}
	else
	{
		current.clear ();
...
lightdag::bulk_pull_client::bulk_pull_client (std::shared_ptr<lightdag::bootstrap_client> connection_a, lightdag::pull_info const & pull_a, size_t size_a) :
connection (connection_a),
pull (pull_a),
size (size_a)
{
...
lightdag::bulk_pull_client::~bulk_pull_client ()
{
...
void lightdag::bulk_pull_client::request ()
{
	expected = pull.head;
	lightdag::bulk_pull req;
	req.start = pull.account;
	req.end = pull.end;
	auto buffer (std::make_shared<std::vector<uint8_t>> ());
	{
		lightdag::vectorstream stream (*buffer);
...
void lightdag::bulk_pull_client::receive_block ()
{
	auto this_l (shared_from_this ());
	connection->start_timeout ();
...
void lightdag::bulk_pull_client::received_type ()
{
	auto this_l (shared_from_this ());
	lightdag::block_type type (static_cast<lightdag::block_type> (connection->receive_buffer[0]));
	switch (type)
	{
		case lightdag::block_type::send:
		{
			connection->start_timeout ();
			boost::asio::async_read (connection->socket, boost::asio::buffer (connection->receive_buffer.data () + 1, lightdag::send_block::size), [this_l](boost::system::error_code const & ec, size_t size_a) {
				this_l->connection->stop_timeout ();
		...
		{
			connection->start_timeout ();
			boost::asio::async_read (connection->socket, boost::asio::buffer (connection->receive_buffer.data () + 1, lightdag::receive_block::size), [this_l](boost::system::error_code const & ec, size_t size_a) {
				this_l->connection->stop_timeout ();
				this_l->received_block (ec, size_a);
			});
			break;
		...
		{
			connection->start_timeout ();
			boost::asio::async_read (connection->socket, boost::asio::buffer (connection->receive_buffer.data () + 1, lightdag::open_block::size), [this_l](boost::system::error_code const & ec, size_t size_a) {
				this_l->connection->stop_timeout ();
				this_l->received_block (ec, size_a);
			});
			break;
		...
		{
			connection->start_timeout ();
			boost::asio::async_read (connection->socket, boost::asio::buffer (connection->receive_buffer.data () + 1, lightdag::change_block::size), [this_l](boost::system::error_code const & ec, size_t size_a) {
				this_l->connection->stop_timeout ();
				this_l->received_block (ec, size_a);
			});
			break;
		...
		{
			connection->start_timeout ();
			boost::asio::async_read (connection->socket, boost::asio::buffer (connection->receive_buffer.data () + 1, lightdag::state_block::size), [this_l](boost::system::error_code const & ec, size_t size_a) {
				this_l->connection->stop_timeout ();
				this_l->received_block (ec, size_a);
			});
			break;
		}
		case lightdag::block_type::not_a_block:
		{
...
void lightdag::bulk_pull_client::received_block (boost::system::error_code const & ec, size_t size_a)
{
	if (!ec)
	{
		lightdag::bufferstream stream (connection->receive_buffer.data (), 1 + size_a);
		std::shared_ptr<lightdag::block> block (lightdag::deserialize_block (stream));
		if (block != nullptr && !lightdag::work_validate (*block))
		{
			...
			connection->attempt->node->block_processor.add (lightdag::block_processor_item (block));
...
lightdag::bulk_push_client::bulk_push_client (std::shared_ptr<lightdag::bootstrap_client> const & connection_a) :
connection (connection_a),
synchronization (*connection->node, [this](MDB_txn * transaction_a, lightdag::block const & block_a) {
	push_block (block_a);
	return lightdag::sync_result::success;
})
{
}

lightdag::bulk_push_client::~bulk_push_client ()
{
}

void lightdag::bulk_push_client::start ()
{
	lightdag::bulk_push message;
	auto buffer (std::make_shared<std::vector<uint8_t>> ());
	{
		lightdag::vectorstream stream (*buffer);
	...
	boost::asio::async_write (connection->socket, boost::asio::buffer (buffer->data (), buffer->size ()), [this_l, buffer](boost::system::error_code const & ec, size_t size_a) {
		this_l->connection->stop_timeout ();
		lightdag::transaction transaction (this_l->connection->node->store.environment, nullptr, true);
		if (!ec)
		{
...
void lightdag::bulk_push_client::push (MDB_txn * transaction_a)
{
	auto finished (false);
	{
		auto first (connection->node->store.unsynced_begin (transaction_a));
		if (first != lightdag::store_iterator (nullptr))
		{
			lightdag::block_hash hash (first->first.uint256 ());
			if (!hash.is_zero ())
			{
...
void lightdag::bulk_push_client::send_finished ()
{
	auto buffer (std::make_shared<std::vector<uint8_t>> ());
	buffer->push_back (static_cast<uint8_t> (lightdag::block_type::not_a_block));
	if (connection->node->config.logging.network_logging ())
	{
...
void lightdag::bulk_push_client::push_block (lightdag::block const & block_a)
{
	auto buffer (std::make_shared<std::vector<uint8_t>> ());
	{
		lightdag::vectorstream stream (*buffer);
		lightdag::serialize_block (stream, block_a);
	}
	auto this_l (shared_from_this ());
	connection->start_timeout ();
	boost::asio::async_write (connection->socket, boost::asio::buffer (buffer->data (), buffer->size ()), [this_l, buffer](boost::system::error_code const & ec, size_t size_a) {
...
lightdag::pull_info::pull_info (lightdag::account const & account_a, lightdag::block_hash const & head_a, lightdag::block_hash const & end_a) :
account (account_a),
head (head_a),
end (end_a),
attempts (0)
...
lightdag::bootstrap_attempt::bootstrap_attempt (std::shared_ptr<lightdag::node> node_a) :
connections (0),
pulling (0),
node (node_a),
account_count (0),
...
lightdag::bootstrap_attempt::~bootstrap_attempt ()
{
	BOOST_LOG (node->log) << "Exiting bootstrap attempt";
	node->bootstrap_initiator.notify_listeners (false);
}

bool lightdag::bootstrap_attempt::request_frontier (std::unique_lock<std::mutex> & lock_a)
{
	auto result (true);
	...
	if (connection_l)
	{
		...
		{
			auto client (std::make_shared<lightdag::frontier_req_client> (connection_l));
			client->run ();
			frontiers = client;
...
void lightdag::bootstrap_attempt::request_pull (std::unique_lock<std::mutex> & lock_a)
{
	auto connection_l (connection (lock_a));
	if (connection_l)
	{
		...
		node->background ([connection_l, pull, size]() {
			auto client (std::make_shared<lightdag::bulk_pull_client> (connection_l, pull, size));
			client->request ();
		});
	}
}

bool lightdag::bootstrap_attempt::request_push (std::unique_lock<std::mutex> & lock_a)
{
	auto result (true);
	auto connection_l (connection (lock_a));
	if (connection_l)
	{
		std::future<bool> future;
		{
			auto client (std::make_shared<lightdag::bulk_push_client> (connection_l));
			client->start ();
			push = client;
			future = client->promise.get_future ();
		}
...
bool lightdag::bootstrap_attempt::still_pulling ()
{
...
void lightdag::bootstrap_attempt::run ()
{
	...
	{
		auto k = lightdag::random_pool.GenerateWord32 (0, i);
...
std::shared_ptr<lightdag::bootstrap_client> lightdag::bootstrap_attempt::connection (std::unique_lock<std::mutex> & lock_a)
{
	while (!stopped && idle.empty ())
	{
		condition.wait (lock_a);
	}
	std::shared_ptr<lightdag::bootstrap_client> result;
	if (!idle.empty ())
	{
		result = idle.back ();
		idle.pop_back ();
...
bool lightdag::bootstrap_attempt::consume_future (std::future<bool> & future_a)
{
	bool result;
	try
	{
...
void lightdag::bootstrap_attempt::process_fork (MDB_txn * transaction_a, std::shared_ptr<lightdag::block> block_a)
{
	try_resolve_fork (transaction_a, block_a, true);
}

void lightdag::bootstrap_attempt::try_resolve_fork (MDB_txn * transaction_a, std::shared_ptr<lightdag::block> block_a, bool from_processor)
{
	std::weak_ptr<lightdag::bootstrap_attempt> this_w (shared_from_this ());
	...
	{
		std::shared_ptr<lightdag::block> ledger_block (node->ledger.forked_block (transaction_a, *block_a));
		if (ledger_block)
		{
			node->active.start (transaction_a, ledger_block, [this_w, block_a](std::shared_ptr<lightdag::block>, bool resolved) {
				if (auto this_l = this_w.lock ())
				{
					if (resolved)
					{
						...
						lightdag::transaction transaction (this_l->node->store.environment, nullptr, false);
						auto account (this_l->node->ledger.store.frontier_get (transaction, block_a->root ()));
						if (!account.is_zero ())
						{
							this_l->requeue_pull (lightdag::pull_info (account, block_a->root (), block_a->root ()));
						}
						else if (this_l->node->ledger.store.account_exists (transaction, block_a->root ()))
						{
							this_l->requeue_pull (lightdag::pull_info (block_a->root (), lightdag::block_hash (0), lightdag::block_hash (0)));
						}
					}
				}
			});
...
void lightdag::bootstrap_attempt::resolve_forks ()
{
	std::unordered_map<lightdag::block_hash, std::shared_ptr<lightdag::block>> forks_to_resolve;
	{
		std::unique_lock<std::mutex> lock (mutex);
...
{
	bool operator() (const std::shared_ptr<lightdag::bootstrap_client> & lhs, const std::shared_ptr<lightdag::bootstrap_client> & rhs) const
	{
		return lhs->block_rate () > rhs->block_rate ();
	}
};

unsigned lightdag::bootstrap_attempt::target_connections (size_t pulls_remaining)
{
...
void lightdag::bootstrap_attempt::populate_connections ()
{
	double rate_sum = 0.0;
	size_t num_pulls = 0;
	std::priority_queue<std::shared_ptr<lightdag::bootstrap_client>, std::vector<std::shared_ptr<lightdag::bootstrap_client>>, block_rate_cmp> sorted_connections;
	{
		std::unique_lock<std::mutex> lock (mutex);
	...
	if (connections < target)
	{
		...
		for (int i = 0; i < delta; i++)
		{
			...
			if (peer != lightdag::endpoint (boost::asio::ip::address_v6::any (), 0))
			{
				auto client (std::make_shared<lightdag::bootstrap_client> (node, shared_from_this (), lightdag::tcp_endpoint (peer.address (), peer.port ())));
				client->run ();
...
void lightdag::bootstrap_attempt::add_connection (lightdag::endpoint const & endpoint_a)
{
	auto client (std::make_shared<lightdag::bootstrap_client> (node, shared_from_this (), lightdag::tcp_endpoint (endpoint_a.address (), endpoint_a.port ())));
	client->run ();
}

void lightdag::bootstrap_attempt::pool_connection (std::shared_ptr<lightdag::bootstrap_client> client_a)
{
	std::lock_guard<std::mutex> lock (mutex);
...
void lightdag::bootstrap_attempt::stop ()
{
	std::lock_guard<std::mutex> lock (mutex);
	stopped = true;
	condition.notify_all ();
...
void lightdag::bootstrap_attempt::add_pull (lightdag::pull_info const & pull)
{
	std::lock_guard<std::mutex> lock (mutex);
...
void lightdag::bootstrap_attempt::requeue_pull (lightdag::pull_info const & pull_a)
{
	auto pull (pull_a);
	...
	else if (pull.attempts == bootstrap_frontier_retry_limit)
	{
		...
		if (auto connection_shared = connection_frontier_request.lock ())
		{
			...
			node->background ([connection_shared, pull, size]() {
				auto client (std::make_shared<lightdag::bulk_pull_client> (connection_shared, pull, size));
				client->request ();
			});
			if (node->config.logging.bulk_pull_logging ())
			{
...
lightdag::bootstrap_initiator::bootstrap_initiator (lightdag::node & node_a) :
node (node_a),
stopped (false),
...
void lightdag::bootstrap_initiator::bootstrap ()
{
	std::unique_lock<std::mutex> lock (mutex);
	if (!stopped && attempt == nullptr)
	{
		attempt = std::make_shared<lightdag::bootstrap_attempt> (node.shared ());
		condition.notify_all ();
	}
}

void lightdag::bootstrap_initiator::bootstrap (lightdag::endpoint const & endpoint_a)
{
...
void lightdag::bootstrap_initiator::run_bootstrap ()
{
	std::unique_lock<std::mutex> lock (mutex);
	while (!stopped)
	{
...
void lightdag::bootstrap_initiator::add_observer (std::function<void(bool)> const & observer_a)
{
	std::lock_guard<std::mutex> lock (mutex);
	observers.push_back (observer_a);
}

bool lightdag::bootstrap_initiator::in_progress ()
{
	std::lock_guard<std::mutex> lock (mutex);
	return attempt != nullptr;
}

void lightdag::bootstrap_initiator::stop ()
{
	std::unique_lock<std::mutex> lock (mutex);
	stopped = true;
	if (attempt != nullptr)
...
void lightdag::bootstrap_initiator::notify_listeners (bool in_progress_a)
{
...
void lightdag::bootstrap_initiator::process_fork (MDB_txn * transaction, std::shared_ptr<lightdag::block> block_a)
{
	std::unique_lock<std::mutex> lock (mutex);
	if (attempt != nullptr)
	{
...
lightdag::bootstrap_listener::bootstrap_listener (boost::asio::io_service & service_a, uint16_t port_a, lightdag::node & node_a) :
acceptor (service_a),
...
void lightdag::bootstrap_listener::start ()
{
...
void lightdag::bootstrap_listener::stop ()
{
	on = false;
	std::lock_guard<std::mutex> lock (mutex);
	acceptor.close ();
...
void lightdag::bootstrap_listener::accept_connection ()
{
...
void lightdag::bootstrap_listener::accept_action (boost::system::error_code const & ec, std::shared_ptr<boost::asio::ip::tcp::socket> socket_a)
{
	if (!ec)
	{
		accept_connection ();
		auto connection (std::make_shared<lightdag::bootstrap_server> (socket_a, node.shared ()));
		{
			std::lock_guard<std::mutex> lock (mutex);
...
boost::asio::ip::tcp::endpoint lightdag::bootstrap_listener::endpoint ()
{
	return boost::asio::ip::tcp::endpoint (boost::asio::ip::address_v6::loopback (), local.port ());
}

lightdag::bootstrap_server::~bootstrap_server ()
{
	if (node->config.logging.bulk_pull_logging ())
	{
...
lightdag::bootstrap_server::bootstrap_server (std::shared_ptr<boost::asio::ip::tcp::socket> socket_a, std::shared_ptr<lightdag::node> node_a) :
socket (socket_a),
node (node_a)
{
}

void lightdag::bootstrap_server::receive ()
{
	auto this_l (shared_from_this ());
...
void lightdag::bootstrap_server::receive_header_action (boost::system::error_code const & ec, size_t size_a)
{
	if (!ec)
	{
		assert (size_a == 8);
		lightdag::bufferstream type_stream (receive_buffer.data (), size_a);
		uint8_t version_max;
		uint8_t version_using;
		uint8_t version_min;
		lightdag::message_type type;
		std::bitset<16> extensions;
		if (!lightdag::message::read_header (type_stream, version_max, version_using, version_min, type, extensions))
		{
			switch (type)
			{
				case lightdag::message_type::bulk_pull:
				{
					auto this_l (shared_from_this ());
					boost::asio::async_read (*socket, boost::asio::buffer (receive_buffer.data () + 8, sizeof (lightdag::uint256_union) + sizeof (lightdag::uint256_union)), [this_l](boost::system::error_code const & ec, size_t size_a) {
						this_l->receive_bulk_pull_action (ec, size_a);
					});
					break;
				}
				case lightdag::message_type::bulk_pull_blocks:
				{
					auto this_l (shared_from_this ());
					boost::asio::async_read (*socket, boost::asio::buffer (receive_buffer.data () + lightdag::bootstrap_message_header_size, sizeof (lightdag::uint256_union) + sizeof (lightdag::uint256_union) + sizeof (bulk_pull_blocks_mode) + sizeof (uint32_t)), [this_l](boost::system::error_code const & ec, size_t size_a) {
						this_l->receive_bulk_pull_blocks_action (ec, size_a);
					});
					break;
				}
				case lightdag::message_type::frontier_req:
				{
					auto this_l (shared_from_this ());
					boost::asio::async_read (*socket, boost::asio::buffer (receive_buffer.data () + 8, sizeof (lightdag::uint256_union) + sizeof (uint32_t) + sizeof (uint32_t)), [this_l](boost::system::error_code const & ec, size_t size_a) {
						this_l->receive_frontier_req_action (ec, size_a);
					});
					break;
				}
				case lightdag::message_type::bulk_push:
				{
					add_request (std::unique_ptr<lightdag::message> (new lightdag::bulk_push));
					break;
				}
				default:
				{
...
void lightdag::bootstrap_server::receive_bulk_pull_action (boost::system::error_code const & ec, size_t size_a)
{
	if (!ec)
	{
		std::unique_ptr<lightdag::bulk_pull> request (new lightdag::bulk_pull);
		lightdag::bufferstream stream (receive_buffer.data (), 8 + sizeof (lightdag::uint256_union) + sizeof (lightdag::uint256_union));
		...
		if (!error)
		{
			...
			add_request (std::unique_ptr<lightdag::message> (request.release ()));
			receive ();
		}
	}
}

void lightdag::bootstrap_server::receive_bulk_pull_blocks_action (boost::system::error_code const & ec, size_t size_a)
{
	if (!ec)
	{
		std::unique_ptr<lightdag::bulk_pull_blocks> request (new lightdag::bulk_pull_blocks);
		lightdag::bufferstream stream (receive_buffer.data (), 8 + sizeof (lightdag::uint256_union) + sizeof (lightdag::uint256_union) + sizeof (bulk_pull_blocks_mode) + sizeof (uint32_t));
		auto error (request->deserialize (stream));
		if (!error)
		{
			if (node->config.logging.bulk_pull_logging ())
...
void lightdag::bootstrap_server::receive_frontier_req_action (boost::system::error_code const & ec, size_t size_a)
{
	if (!ec)
	{
		std::unique_ptr<lightdag::frontier_req> request (new lightdag::frontier_req);
		lightdag::bufferstream stream (receive_buffer.data (), 8 + sizeof (lightdag::uint256_union) + sizeof (uint32_t) + sizeof (uint32_t));
		auto error (request->deserialize (stream));
		if (!error)
		{
			if (node->config.logging.bulk_pull_logging ())
...
void lightdag::bootstrap_server::add_request (std::unique_ptr<lightdag::message> message_a)
{
	std::lock_guard<std::mutex> lock (mutex);
...
void lightdag::bootstrap_server::finish_request ()
{
	std::lock_guard<std::mutex> lock (mutex);
	requests.pop ();
	if (!requests.empty ())
...
namespace
{
class request_response_visitor : public lightdag::message_visitor
{
public:
	request_response_visitor (std::shared_ptr<lightdag::bootstrap_server> connection_a) :
	connection (connection_a)
	{
	}
	virtual ~request_response_visitor () = default;
	void keepalive (lightdag::keepalive const &) override
	{
		assert (false);
	}
	void publish (lightdag::publish const &) override
	{
		assert (false);
	}
	void confirm_req (lightdag::confirm_req const &) override
	{
		assert (false);
	}
	void confirm_ack (lightdag::confirm_ack const &) override
	{
		assert (false);
	}
	void bulk_pull (lightdag::bulk_pull const &) override
	{
		auto response (std::make_shared<lightdag::bulk_pull_server> (connection, std::unique_ptr<lightdag::bulk_pull> (static_cast<lightdag::bulk_pull *> (connection->requests.front ().release ()))));
		response->send_next ();
	}
	void bulk_pull_blocks (lightdag::bulk_pull_blocks const &) override
	{
		auto response (std::make_shared<lightdag::bulk_pull_blocks_server> (connection, std::unique_ptr<lightdag::bulk_pull_blocks> (static_cast<lightdag::bulk_pull_blocks *> (connection->requests.front ().release ()))));
		response->send_next ();
	}
	void bulk_push (lightdag::bulk_push const &) override
	{
		auto response (std::make_shared<lightdag::bulk_push_server> (connection));
		response->receive ();
	}
	void frontier_req (lightdag::frontier_req const &) override
	{
		auto response (std::make_shared<lightdag::frontier_req_server> (connection, std::unique_ptr<lightdag::frontier_req> (static_cast<lightdag::frontier_req *> (connection->requests.front ().release ()))));
		response->send_next ();
	}
...
void lightdag::bootstrap_server::run_next ()
{
 ...
 */
void lightdag::bulk_pull_server::set_current_end ()
{
...
void lightdag::bulk_pull_server::send_next ()
{
	std::unique_ptr<lightdag::block> block (get_next ());
	if (block != nullptr)
	{
		{
			send_buffer.clear ();
			lightdag::vectorstream stream (send_buffer);
			lightdag::serialize_block (stream, *block);
		}
		auto this_l (shared_from_this ());
		if (connection->node->config.logging.bulk_pull_logging ())
		{
...
std::unique_ptr<lightdag::block> lightdag::bulk_pull_server::get_next ()
{
...
void lightdag::bulk_pull_server::sent_action (boost::system::error_code const & ec, size_t size_a)
{
	if (!ec)
	{
		send_next ();
...
void lightdag::bulk_pull_server::send_finished ()
{
	send_buffer.clear ();
	send_buffer.push_back (static_cast<uint8_t> (lightdag::block_type::not_a_block));
	auto this_l (shared_from_this ());
	if (connection->node->config.logging.bulk_pull_logging ())
	{
...
void lightdag::bulk_pull_server::no_block_sent (boost::system::error_code const & ec, size_t size_a)
{
	if (!ec)
	{
		assert (size_a == 1);
...
lightdag::bulk_pull_server::bulk_pull_server (std::shared_ptr<lightdag::bootstrap_server> const & connection_a, std::unique_ptr<lightdag::bulk_pull> request_a) :
connection (connection_a),
 ...
 */
void lightdag::bulk_pull_blocks_server::set_params ()
{
	assert (request != nullptr);
	...
	if (connection->node->config.logging.bulk_pull_logging ())
	{
		...
		switch (request->mode)
		{
			case lightdag::bulk_pull_blocks_mode::list_blocks:
				modeName = "list";
				break;
			case lightdag::bulk_pull_blocks_mode::checksum_blocks:
				modeName = "checksum";
				break;
		}
...
void lightdag::bulk_pull_blocks_server::send_next ()
{
	std::unique_ptr<lightdag::block> block (get_next ());
	if (block != nullptr)
	{
		...
		if (request->mode == lightdag::bulk_pull_blocks_mode::list_blocks)
		{
			lightdag::vectorstream stream (send_buffer);
			lightdag::serialize_block (stream, *block);
		}
		else if (request->mode == lightdag::bulk_pull_blocks_mode::checksum_blocks)
		{
			checksum ^= block->hash ();
		}
	...
	else
	{
		...
		if (request->mode == lightdag::bulk_pull_blocks_mode::checksum_blocks)
		{
			{
				send_buffer.clear ();
				lightdag::vectorstream stream (send_buffer);
				write (stream, static_cast<uint8_t> (lightdag::block_type::not_a_block));
...
std::unique_ptr<lightdag::block> lightdag::bulk_pull_blocks_server::get_next ()
{
	std::unique_ptr<lightdag::block> result;
	bool out_of_bounds;
...
void lightdag::bulk_pull_blocks_server::sent_action (boost::system::error_code const & ec, size_t size_a)
{
	if (!ec)
	{
		send_next ();
...
void lightdag::bulk_pull_blocks_server::send_finished ()
{
	send_buffer.clear ();
	send_buffer.push_back (static_cast<uint8_t> (lightdag::block_type::not_a_block));
	auto this_l (shared_from_this ());
...
void lightdag::bulk_pull_blocks_server::no_block_sent (boost::system::error_code const & ec, size_t size_a)
{
	if (!ec)
	{
		assert (size_a == 1);
...
lightdag::bulk_pull_blocks_server::bulk_pull_blocks_server (std::shared_ptr<lightdag::bootstrap_server> const & connection_a, std::unique_ptr<lightdag::bulk_pull_blocks> request_a) :
connection (connection_a),
request (std::move (request_a)),
stream (nullptr),
...
lightdag::bulk_push_server::bulk_push_server (std::shared_ptr<lightdag::bootstrap_server> const & connection_a) :
connection (connection_a)
{
}

void lightdag::bulk_push_server::receive ()
{
	auto this_l (shared_from_this ());
...
void lightdag::bulk_push_server::received_type ()
{
	auto this_l (shared_from_this ());
	lightdag::block_type type (static_cast<lightdag::block_type> (receive_buffer[0]));
	switch (type)
	{
		case lightdag::block_type::send:
		{
			boost::asio::async_read (*connection->socket, boost::asio::buffer (receive_buffer.data () + 1, lightdag::send_block::size), [this_l](boost::system::error_code const & ec, size_t size_a) {
				this_l->received_block (ec, size_a);
			});
			break;
		}
		case lightdag::block_type::receive:
		{
			boost::asio::async_read (*connection->socket, boost::asio::buffer (receive_buffer.data () + 1, lightdag::receive_block::size), [this_l](boost::system::error_code const & ec, size_t size_a) {
				this_l->received_block (ec, size_a);
			});
			break;
		}
		case lightdag::block_type::open:
		{
			boost::asio::async_read (*connection->socket, boost::asio::buffer (receive_buffer.data () + 1, lightdag::open_block::size), [this_l](boost::system::error_code const & ec, size_t size_a) {
				this_l->received_block (ec, size_a);
			});
			break;
		}
		case lightdag::block_type::change:
		{
			boost::asio::async_read (*connection->socket, boost::asio::buffer (receive_buffer.data () + 1, lightdag::change_block::size), [this_l](boost::system::error_code const & ec, size_t size_a) {
				this_l->received_block (ec, size_a);
			});
			break;
		}
		case lightdag::block_type::state:
		{
			boost::asio::async_read (*connection->socket, boost::asio::buffer (receive_buffer.data () + 1, lightdag::state_block::size), [this_l](boost::system::error_code const & ec, size_t size_a) {
				this_l->received_block (ec, size_a);
			});
			break;
		}
...
void lightdag::bulk_push_server::received_block (boost::system::error_code const & ec, size_t size_a)
{
	if (!ec)
	{
		lightdag::bufferstream stream (receive_buffer.data (), 1 + size_a);
		auto block (lightdag::deserialize_block (stream));
		if (block != nullptr && !lightdag::work_validate (*block))
		{
...
lightdag::frontier_req_server::frontier_req_server (std::shared_ptr<lightdag::bootstrap_server> const & connection_a, std::unique_ptr<lightdag::frontier_req> request_a) :
connection (connection_a),
...
void lightdag::frontier_req_server::skip_old ()
{
	if (request->age != std::numeric_limits<decltype (request->age)>::max ())
	{
		auto now (lightdag::seconds_since_epoch ());
...
void lightdag::frontier_req_server::send_next ()
{
	if (!current.is_zero ())
	{
		{
...
void lightdag::frontier_req_server::send_finished ()
{
	{
		send_buffer.clear ();
		lightdag::vectorstream stream (send_buffer);
		lightdag::uint256_union zero (0);
...
void lightdag::frontier_req_server::no_block_sent (boost::system::error_code const & ec, size_t size_a)
{
	if (!ec)
	{
		connection->finish_request ();
...
void lightdag::frontier_req_server::sent_action (boost::system::error_code const & ec, size_t size_a)
{
	if (!ec)
	{
		send_next ();
...
void lightdag::frontier_req_server::next ()
{
	lightdag::transaction transaction (connection->node->store.environment, nullptr, false);
	auto iterator (connection->node->store.latest_begin (transaction, current.number () + 1));
	if (iterator != connection->node->store.latest_end ())
	{
		current = lightdag::uint256_union (iterator->first.uint256 ());
		info = lightdag::account_info (iterator->second);
	}
	else
	{
