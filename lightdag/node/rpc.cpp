...
#ifdef LIGHTDAG_SECURE_RPC
#include <lightdag/node/rpc_secure.hpp>
#endif

lightdag::rpc_secure_config::rpc_secure_config () :
enable (false),
verbose_logging (false)
{
}

void lightdag::rpc_secure_config::serialize_json (boost::property_tree::ptree & tree_a) const
{
...
bool lightdag::rpc_secure_config::deserialize_json (boost::property_tree::ptree const & tree_a)
{
	auto error (false);
	try
	{
...
lightdag::rpc_config::rpc_config () :
address (boost::asio::ip::address_v6::loopback ()),
port (lightdag::rpc::rpc_port),
...
lightdag::rpc_config::rpc_config (bool enable_control_a) :
address (boost::asio::ip::address_v6::loopback ()),
port (lightdag::rpc::rpc_port),
...
void lightdag::rpc_config::serialize_json (boost::property_tree::ptree & tree_a) const
{
...
bool lightdag::rpc_config::deserialize_json (boost::property_tree::ptree const & tree_a)
{
	auto result (false);
	try
	{
...
lightdag::rpc::rpc (boost::asio::io_service & service_a, lightdag::node & node_a, lightdag::rpc_config const & config_a) :
acceptor (service_a),
config (config_a),
node (node_a)
{
...
void lightdag::rpc::start ()
{
	auto endpoint (lightdag::tcp_endpoint (config.address, config.port));
	...
	acceptor.listen ();
	node.observers.blocks.add ([this](std::shared_ptr<lightdag::block> block_a, lightdag::process_return const & result_a) {
...
void lightdag::rpc::accept ()
{
	auto connection (std::make_shared<lightdag::rpc_connection> (node, *this));
...
void lightdag::rpc::stop ()
{
	acceptor.close ();
}

lightdag::rpc_handler::rpc_handler (lightdag::node & node_a, lightdag::rpc & rpc_a, std::string const & body_a, std::function<void(boost::property_tree::ptree const &)> const & response_a) :
body (body_a),
node (node_a),
rpc (rpc_a),
response (response_a)
...
void lightdag::rpc::observer_action (lightdag::account const & account_a)
{
	std::shared_ptr<lightdag::payment_observer> observer;
	{
...
void lightdag::error_response (std::function<void(boost::property_tree::ptree const &)> response_a, std::string const & message_a)
{
...
void lightdag::rpc_handler::account_balance ()
{
...
void lightdag::rpc_handler::account_block_count ()
{
	std::string account_text (request.get<std::string> ("account"));
	lightdag::uint256_union account;
	auto error (account.decode_account (account_text));
...
void lightdag::rpc_handler::account_create ()
{
	if (rpc.config.enable_control)
	{
		std::string wallet_text (request.get<std::string> ("wallet"));
		lightdag::uint256_union wallet;
		...
		if (!error)
		{
			...
			if (existing != node.wallets.items.end ())
			{
				...
				lightdag::account new_key (existing->second->deterministic_insert (generate_work));
				if (!new_key.is_zero ())
				{
					boost::property_tree::ptree response_l;
...
void lightdag::rpc_handler::account_get ()
{
...
void lightdag::rpc_handler::account_info ()
{
	std::string account_text (request.get<std::string> ("account"));
	lightdag::uint256_union account;
	auto error (account.decode_account (account_text));
	if (!error)
	{
		...
		if (!node.store.account_get (transaction, account, info))
		{
			...
			std::string balance;
			lightdag::uint128_union (info.balance).encode_dec (balance);
...
void lightdag::rpc_handler::account_key ()
{
	std::string account_text (request.get<std::string> ("account"));
...
void lightdag::rpc_handler::account_list ()
{
	std::string wallet_text (request.get<std::string> ("wallet"));
	lightdag::uint256_union wallet;
	auto error (wallet.decode_hex (wallet_text));
	if (!error)
	{
		...
		if (existing != node.wallets.items.end ())
		{
			...
			for (auto i (existing->second->store.begin (transaction)), j (existing->second->store.end ()); i != j; ++i)
			{
				...
				entry.put ("", lightdag::uint256_union (i->first.uint256 ()).to_account ());
...
void lightdag::rpc_handler::account_move ()
{
	if (rpc.config.enable_control)
	{
		std::string wallet_text (request.get<std::string> ("wallet"));
			...
			if (existing != node.wallets.items.end ())
			{
				...
				if (!error)
				{
					...
					if (existing != node.wallets.items.end ())
					{
						...
						std::vector<lightdag::public_key> accounts;
						for (auto i (accounts_text.begin ()), n (accounts_text.end ()); i != n; ++i)
						{
							lightdag::public_key account;
...
void lightdag::rpc_handler::account_remove ()
{
	if (rpc.config.enable_control)
	{
		std::string wallet_text (request.get<std::string> ("wallet"));
...
void lightdag::rpc_handler::account_representative ()
{
	std::string account_text (request.get<std::string> ("account"));
	lightdag::account account;
	auto error (account.decode_account (account_text));
...
void lightdag::rpc_handler::account_representative_set ()
{
	if (rpc.config.enable_control)
	{
		std::string wallet_text (request.get<std::string> ("wallet"));
			...
			if (existing != node.wallets.items.end ())
			{
				...
				if (!error)
				{
					...
					if (!error)
					{
						...
						{
							auto work_error (lightdag::from_string_hex (work_text.get (), work));
							if (work_error)
							{
						...
						if (work)
						{
							...
							if (!node.store.account_get (transaction, account, info))
							{
								if (!lightdag::work_validate (info.head, work))
								{
						...
						wallet->change_async (account, representative, [response_a](std::shared_ptr<lightdag::block> block) {
							lightdag::block_hash hash (0);
							if (block != nullptr)
							{
								hash = block->hash ();
							}
...
void lightdag::rpc_handler::account_weight ()
{
	std::string account_text (request.get<std::string> ("account"));
	lightdag::uint256_union account;
	auto error (account.decode_account (account_text));
...
void lightdag::rpc_handler::accounts_balances ()
{
	boost::property_tree::ptree response_l;
...
void lightdag::rpc_handler::accounts_create ()
{
	if (rpc.config.enable_control)
	{
		std::string wallet_text (request.get<std::string> ("wallet"));
...
void lightdag::rpc_handler::accounts_frontiers ()
{
	boost::property_tree::ptree response_l;
...
void lightdag::rpc_handler::accounts_pending ()
{
	uint64_t count (std::numeric_limits<uint64_t>::max ());
	lightdag::uint128_union threshold (0);
	...
	for (auto & accounts : request.get_child ("accounts"))
	{
		...
		if (!account.decode_account (account_text))
		{
			...
			lightdag::account end (account.number () + 1);
			for (auto i (node.store.pending_begin (transaction, lightdag::pending_key (account, 0))), n (node.store.pending_begin (transaction, lightdag::pending_key (end, 0))); i != n && peers_l.size () < count; ++i)
			{
...
void lightdag::rpc_handler::available_supply ()
{
	auto genesis_balance (node.balance (lightdag::genesis_account)); // Cold storage genesis
	auto landing_balance (node.balance (lightdag::account ("059F68AAB29DE0D3A27443625C7EA9CDDB6517A8B76FE37727EF6A4D76832AD5"))); // Active unavailable account
	auto faucet_balance (node.balance (lightdag::account ("8E319CE6F3025E5B2DF66DA7AB1467FE48F1679C13DD43BFDB29FA2E9FC40D3B"))); // Faucet account
	auto burned_balance ((node.balance_pending (lightdag::account (0))).second); // Burning 0 account
	auto available (lightdag::genesis_amount - genesis_balance - landing_balance - faucet_balance - burned_balance);
	boost::property_tree::ptree response_l;
	response_l.put ("available", available.convert_to<std::string> ());
	response (response_l);
}

void lightdag::rpc_handler::block ()
{
	std::string hash_text (request.get<std::string> ("hash"));
	lightdag::uint256_union hash;
...
void lightdag::rpc_handler::blocks ()
{
...
void lightdag::rpc_handler::blocks_info ()
{
	const bool pending = request.get<bool> ("pending", false);
	const bool source = request.get<bool> ("source", false);
	std::vector<std::string> hashes;
		...
		if (!error)
		{
			...
			if (block != nullptr)
			{
				...
				if (pending)
				{
					...
					{
						exists = node.store.pending_exists (transaction, lightdag::pending_key (destination, hash));
					}
					entry.put ("pending", exists ? "1" : "0");
				}
				if (source)
				{
					lightdag::block_hash source_hash (node.ledger.block_source (transaction, *block));
					std::unique_ptr<lightdag::block> block_a (node.store.block_get (transaction, source_hash));
					if (block_a != nullptr)
					{
...
void lightdag::rpc_handler::block_account ()
{
	std::string hash_text (request.get<std::string> ("hash"));
...
void lightdag::rpc_handler::block_count ()
{
	lightdag::transaction transaction (node.store.environment, nullptr, false);
	boost::property_tree::ptree response_l;
...
void lightdag::rpc_handler::block_count_type ()
{
	lightdag::transaction transaction (node.store.environment, nullptr, false);
	lightdag::block_counts count (node.store.block_count (transaction));
	boost::property_tree::ptree response_l;
...
void lightdag::rpc_handler::block_create ()
{
	if (rpc.config.enable_control)
	{
		std::string type (request.get<std::string> ("type"));
		lightdag::uint256_union wallet (0);
		...
		lightdag::uint256_union account (0);
		...
		lightdag::uint256_union representative (0);
		...
		lightdag::uint256_union destination (0);
		...
		lightdag::uint128_union amount (0);
		...
		prv.data.clear ();
		lightdag::uint256_union previous (0);
		lightdag::uint128_union balance (0);
		lightdag::uint256_union link (0);
		...
		if (prv.data != 0)
		{
			...
			if (type == "state")
			{
				...
				{
					lightdag::state_block state (account, previous, representative, balance, link, prv, pub, work);
					boost::property_tree::ptree response_l;
			...
			else if (type == "open")
			{
				if (representative != 0 && source != 0)
				{
					...
					lightdag::open_block open (source, representative, pub, prv, pub, work);
					boost::property_tree::ptree response_l;
			...
			else if (type == "receive")
			{
				if (source != 0 && previous != 0)
				{
					...
					lightdag::receive_block receive (previous, source, prv, pub, work);
					boost::property_tree::ptree response_l;
			...
			else if (type == "change")
			{
				if (representative != 0 && previous != 0)
				{
					...
					lightdag::change_block change (previous, representative, prv, pub, work);
					boost::property_tree::ptree response_l;
			...
			else if (type == "send")
			{
				if (destination != 0 && previous != 0 && balance != 0 && amount != 0)
				{
					if (balance.number () >= amount.number ())
					{
						...
						lightdag::send_block send (previous, destination, balance.number () - amount.number (), prv, pub, work);
						boost::property_tree::ptree response_l;
...
void lightdag::rpc_handler::successors ()
{
	std::string block_text (request.get<std::string> ("block"));
	std::string count_text (request.get<std::string> ("count"));
	lightdag::block_hash block;
...
void lightdag::rpc_handler::bootstrap ()
{
	...
	{
		uint16_t port;
		if (!lightdag::parse_port (port_text, port))
		{
			node.bootstrap_initiator.bootstrap (lightdag::endpoint (address, port));
			boost::property_tree::ptree response_l;
...
void lightdag::rpc_handler::bootstrap_any ()
{
...
void lightdag::rpc_handler::chain ()
{
	std::string block_text (request.get<std::string> ("block"));
	std::string count_text (request.get<std::string> ("count"));
	lightdag::block_hash block;
...
void lightdag::rpc_handler::delegators ()
{
	std::string account_text (request.get<std::string> ("account"));
	lightdag::account account;
	auto error (account.decode_account (account_text));
	if (!error)
	{
		...
		for (auto i (node.store.latest_begin (transaction)), n (node.store.latest_end ()); i != n; ++i)
		{
			...
			if (block->representative () == account)
			{
				std::string balance;
				lightdag::uint128_union (info.balance).encode_dec (balance);
				delegators.put (lightdag::account (i->first.uint256 ()).to_account (), balance);
			}
		}
...
void lightdag::rpc_handler::delegators_count ()
{
	std::string account_text (request.get<std::string> ("account"));
	lightdag::account account;
	auto error (account.decode_account (account_text));
...
void lightdag::rpc_handler::deterministic_key ()
{
	...
	if (!error)
	{
		...
		{
			lightdag::uint256_union index (index_a);
...
void lightdag::rpc_handler::frontiers ()
{
	std::string account_text (request.get<std::string> ("account"));
	std::string count_text (request.get<std::string> ("count"));
	...
	if (!start.decode_account (account_text))
	{
		...
		if (!decode_unsigned (count_text, count))
		{
			...
			{
				frontiers.put (lightdag::account (i->first.uint256 ()).to_account (), lightdag::account_info (i->second).head.to_string ());
			}
			response_l.add_child ("frontiers", frontiers);
			response (response_l);
		}
...
void lightdag::rpc_handler::frontier_count ()
{
	lightdag::transaction transaction (node.store.environment, nullptr, false);
...
namespace
{
class history_visitor : public lightdag::block_visitor
{
public:
	history_visitor (lightdag::rpc_handler & handler_a, bool raw_a, lightdag::transaction & transaction_a, boost::property_tree::ptree & tree_a, lightdag::block_hash const & hash_a) :
	raw (raw_a),
	handler (handler_a),
	...
	void send_block (lightdag::send_block const & block_a)
	{
		tree.put ("type", "send");
	...
	void receive_block (lightdag::receive_block const & block_a)
	{
	...
	void open_block (lightdag::open_block const & block_a)
	{
		if (raw)
		{
			tree.put ("type", "open");
		...
		{
			tree.put ("account", lightdag::genesis_account.to_account ());
			tree.put ("amount", lightdag::genesis_amount.convert_to<std::string> ());
		}
	}
	void change_block (lightdag::change_block const & block_a)
	{
		if (raw)
		{
	...
	void state_block (lightdag::state_block const & block_a)
	{
		if (raw)
		{
			tree.put ("type", "state");
	...
	lightdag::rpc_handler & handler;
	bool raw;
	lightdag::transaction & transaction;
	boost::property_tree::ptree & tree;
	lightdag::block_hash const & hash;
};
}

void lightdag::rpc_handler::account_history ()
{
	std::string count_text (request.get<std::string> ("count"));
...
void lightdag::rpc_handler::keepalive ()
{
	if (rpc.config.enable_control)
	{
		std::string address_text (request.get<std::string> ("address"));
...
void lightdag::rpc_handler::key_create ()
{
	boost::property_tree::ptree response_l;
...
void lightdag::rpc_handler::key_expand ()
{
	std::string key_text (request.get<std::string> ("key"));
	lightdag::uint256_union prv;
...
void lightdag::rpc_handler::ledger ()
{
	if (rpc.config.enable_control)
	{
		lightdag::account start (0);
		uint64_t count (std::numeric_limits<uint64_t>::max ());
		boost::optional<std::string> account_text (request.get_optional<std::string> ("account"));
		if (account_text.is_initialized ())
		{
		...
		{
			std::vector<std::pair<lightdag::uint128_union, lightdag::account>> ledger_l;
			for (auto i (node.store.latest_begin (transaction, start)), n (node.store.latest_end ()); i != n; ++i)
			{
				lightdag::account_info info (i->second);
				lightdag::uint128_union balance (info.balance);
				if (info.modified >= modified_since)
				{
					ledger_l.push_back (std::make_pair (balance, lightdag::account (i->first.uint256 ())));
				}
			}
			...
			for (auto i (ledger_l.begin ()), n (ledger_l.end ()); i != n && accounts.size () < count; ++i)
			{
				...
				lightdag::account account (i->second);
				response_l.put ("frontier", info.head.to_string ());
				response_l.put ("open_block", info.open_block.to_string ());
				response_l.put ("representative_block", info.rep_block.to_string ());
				std::string balance;
...
void lightdag::rpc_handler::mlightdag_from_raw ()
{
	std::string amount_text (request.get<std::string> ("amount"));
	lightdag::uint128_union amount;
	if (!amount.decode_dec (amount_text))
	{
		auto result (amount.number () / lightdag::Mldg_ratio);
		boost::property_tree::ptree response_l;
...
void lightdag::rpc_handler::mlightdag_to_raw ()
{
	std::string amount_text (request.get<std::string> ("amount"));
	lightdag::uint128_union amount;
	if (!amount.decode_dec (amount_text))
...
void lightdag::rpc_handler::klightdag_from_raw ()
{
	std::string amount_text (request.get<std::string> ("amount"));
	lightdag::uint128_union amount;
	if (!amount.decode_dec (amount_text))
	{
		auto result (amount.number () / lightdag::kldg_ratio);
		boost::property_tree::ptree response_l;
		response_l.put ("amount", result.convert_to<std::string> ());
		response (response_l);
	}
...
void lightdag::rpc_handler::klightdag_to_raw ()
{
	std::string amount_text (request.get<std::string> ("amount"));
	lightdag::uint128_union amount;
	if (!amount.decode_dec (amount_text))
...
void lightdag::rpc_handler::password_change ()
{
	if (rpc.config.enable_control)
	{
		std::string wallet_text (request.get<std::string> ("wallet"));
...
void lightdag::rpc_handler::password_enter ()
{
	std::string wallet_text (request.get<std::string> ("wallet"));
	lightdag::uint256_union wallet;
	auto error (wallet.decode_hex (wallet_text));
...
void lightdag::rpc_handler::password_valid (bool wallet_locked = false)
{
	std::string wallet_text (request.get<std::string> ("wallet"));
	lightdag::uint256_union wallet;
	auto error (wallet.decode_hex (wallet_text));
...
void lightdag::rpc_handler::peers ()
{
	boost::property_tree::ptree response_l;
	boost::property_tree::ptree peers_l;
...
void lightdag::rpc_handler::pending ()
{
	std::string account_text (request.get<std::string> ("account"));
	lightdag::account account;
	if (!account.decode_account (account_text))
...
void lightdag::rpc_handler::pending_exists ()
{
	std::string hash_text (request.get<std::string> ("hash"));
	lightdag::uint256_union hash;
	auto error (hash.decode_hex (hash_text));
...
void lightdag::rpc_handler::payment_begin ()
{
	...
	if (!id.decode_hex (id_text))
	{
		...
		{
			lightdag::transaction transaction (node.store.environment, nullptr, true);
			std::shared_ptr<lightdag::wallet> wallet (existing->second);
			if (wallet->store.valid_password (transaction))
			{
				lightdag::account account (0);
				do
				{
...
void lightdag::rpc_handler::payment_init ()
{
	std::string id_text (request.get<std::string> ("wallet"));
	lightdag::uint256_union id;
	if (!id.decode_hex (id_text))
...
void lightdag::rpc_handler::payment_end ()
{
	std::string id_text (request.get<std::string> ("wallet"));
	std::string account_text (request.get<std::string> ("account"));
	lightdag::uint256_union id;
...
void lightdag::rpc_handler::payment_wait ()
{
	std::string account_text (request.get<std::string> ("account"));
	std::string amount_text (request.get<std::string> ("amount"));
	...
	if (!account.decode_account (account_text))
	{
		...
		if (!amount.decode_dec (amount_text))
		{
			...
			{
				{
					auto observer (std::make_shared<lightdag::payment_observer> (response, rpc, account, amount));
...
void lightdag::rpc_handler::process ()
{
	std::string block_text (request.get<std::string> ("block"));
	boost::property_tree::ptree block_l;
	std::stringstream block_stream (block_text);
	boost::property_tree::read_json (block_stream, block_l);
	auto block (lightdag::deserialize_block_json (block_l));
	if (block != nullptr)
	{
		if (!lightdag::work_validate (*block))
		{
			auto hash (block->hash ());
			node.block_arrival.add (hash);
			lightdag::process_return result;
			std::shared_ptr<lightdag::block> block_a (std::move (block));
			{
				lightdag::transaction transaction (node.store.environment, nullptr, true);
...
void lightdag::rpc_handler::lightdag_from_raw ()
{
	std::string amount_text (request.get<std::string> ("amount"));
	lightdag::uint128_union amount;
	if (!amount.decode_dec (amount_text))
	{
		auto result (amount.number () / lightdag::ldg_ratio);
		boost::property_tree::ptree response_l;
		response_l.put ("amount", result.convert_to<std::string> ());
		response (response_l);
	}
...
void lightdag::rpc_handler::lightdag_to_raw ()
{
	std::string amount_text (request.get<std::string> ("amount"));
	lightdag::uint128_union amount;
	if (!amount.decode_dec (amount_text))
...
void lightdag::rpc_handler::receive ()
{
	if (rpc.config.enable_control)
	{
		std::string wallet_text (request.get<std::string> ("wallet"));
				...
				if (!error)
				{
					...
					if (account_check != existing->second->store.end ())
					{
						...
						if (!error)
						{
							...
							if (block != nullptr)
							{
								if (node.store.pending_exists (transaction, lightdag::pending_key (account, hash)))
								{
									uint64_t work (0);
									boost::optional<std::string> work_text (request.get_optional<std::string> ("work"));
									if (work_text.is_initialized ())
									...
									if (work)
									{
										...
										if (!lightdag::work_validate (head, work))
										{
											lightdag::transaction transaction_a (node.store.environment, nullptr, true);
									...
									auto response_a (response);
									existing->second->receive_async (std::move (block), account, lightdag::genesis_amount, [response_a](std::shared_ptr<lightdag::block> block_a) {
										lightdag::uint256_union hash_a (0);
										if (block_a != nullptr)
										{
											hash_a = block_a->hash ();
										}
...
void lightdag::rpc_handler::receive_minimum ()
{
	if (rpc.config.enable_control)
	{
		boost::property_tree::ptree response_l;
...
void lightdag::rpc_handler::receive_minimum_set ()
{
	if (rpc.config.enable_control)
	{
		std::string amount_text (request.get<std::string> ("amount"));
...
void lightdag::rpc_handler::representatives ()
{
	uint64_t count (std::numeric_limits<uint64_t>::max ());
	boost::optional<std::string> count_text (request.get_optional<std::string> ("count"));
	if (count_text.is_initialized ())
	...
	{
		std::vector<std::pair<lightdag::uint128_union, std::string>> representation;
...
void lightdag::rpc_handler::republish ()
{
	...
	if (!error)
	{
		...
		if (block != nullptr)
		{
			for (auto i (0); !hash.is_zero () && i < count; ++i)
			{
				...
				{
					lightdag::block_hash source (node.ledger.block_source (transaction, *block));
					std::unique_ptr<lightdag::block> block_a (node.store.block_get (transaction, source));
					std::vector<lightdag::block_hash> hashes;
				...
				if (destinations != 0) // Republish destination chain
				{
					...
					if (!destination.is_zero ())
					{
						auto exists (node.store.pending_exists (transaction, lightdag::pending_key (destination, hash)));
						if (!exists)
						{
							lightdag::block_hash previous (node.ledger.latest (transaction, destination));
							std::unique_ptr<lightdag::block> block_d (node.store.block_get (transaction, previous));
...
void lightdag::rpc_handler::search_pending ()
{
	if (rpc.config.enable_control)
	{
		std::string wallet_text (request.get<std::string> ("wallet"));
...
void lightdag::rpc_handler::search_pending_all ()
{
	if (rpc.config.enable_control)
	{
...
void lightdag::rpc_handler::send ()
{
	if (rpc.config.enable_control)
	{
		std::string wallet_text (request.get<std::string> ("wallet"));
				...
				if (!error)
				{
					...
					if (!error)
					{
						...
						if (!error)
						{
							...
							lightdag::uint128_t balance (0);
							{
								lightdag::transaction transaction (node.store.environment, nullptr, work != 0); // false if no "work" in request, true if work > 0
								lightdag::account_info info;
							...
							if (balance >= amount.number ())
							{
								...
								auto response_a (response);
								existing->second->send_async (source, destination, amount.number (), [response_a](std::shared_ptr<lightdag::block> block_a) {
									if (block_a != nullptr)
									{
										lightdag::uint256_union hash (block_a->hash ());
										boost::property_tree::ptree response_l;
										response_l.put ("block", hash.to_string ());
										response_a (response_l);
									}
...
void lightdag::rpc_handler::stop ()
{
	if (rpc.config.enable_control)
	{
		boost::property_tree::ptree response_l;
...
void lightdag::rpc_handler::unchecked ()
{
	uint64_t count (std::numeric_limits<uint64_t>::max ());
	boost::optional<std::string> count_text (request.get_optional<std::string> ("count"));
	if (count_text.is_initialized ())
...
void lightdag::rpc_handler::unchecked_clear ()
{
	if (rpc.config.enable_control)
	{
		lightdag::transaction transaction (node.store.environment, nullptr, true);
...
void lightdag::rpc_handler::unchecked_get ()
{
	std::string hash_text (request.get<std::string> ("hash"));
	lightdag::uint256_union hash;
	auto error (hash.decode_hex (hash_text));
...
void lightdag::rpc_handler::unchecked_keys ()
{
	uint64_t count (std::numeric_limits<uint64_t>::max ());
	...
	for (auto i (node.store.unchecked_begin (transaction, key)), n (node.store.unchecked_end ()); i != n && unchecked.size () < count; ++i)
	{
		...
		std::string contents;
		block->serialize_json (contents);
		entry.put ("key", lightdag::block_hash (i->first.uint256 ()).to_string ());
...
void lightdag::rpc_handler::version ()
{
	boost::property_tree::ptree response_l;
	response_l.put ("rpc_version", "1");
	response_l.put ("store_version", std::to_string (node.store_version ()));
	response_l.put ("node_vendor", boost::str (boost::format ("LightDAG %1%.%2%") % LIGHTDAG_VERSION_MAJOR % LIGHTDAG_VERSION_MINOR));
	response (response_l);
}

void lightdag::rpc_handler::validate_account_number ()
{
	std::string account_text (request.get<std::string> ("account"));
	lightdag::uint256_union account;
	auto error (account.decode_account (account_text));
...
void lightdag::rpc_handler::wallet_add ()
{
	if (rpc.config.enable_control)
	{
		std::string key_text (request.get<std::string> ("key"));
...
void lightdag::rpc_handler::wallet_add_watch ()
{
	if (rpc.config.enable_control)
	{
		std::string wallet_text (request.get<std::string> ("wallet"));
...
void lightdag::rpc_handler::wallet_balance_total ()
{
	std::string wallet_text (request.get<std::string> ("wallet"));
	lightdag::uint256_union wallet;
	auto error (wallet.decode_hex (wallet_text));
...
void lightdag::rpc_handler::wallet_balances ()
{
	std::string wallet_text (request.get<std::string> ("wallet"));
	lightdag::uint256_union wallet;
	auto error (wallet.decode_hex (wallet_text));
	if (!error)
	{
		...
		if (existing != node.wallets.items.end ())
		{
			...
			{
				lightdag::account account (i->first.uint256 ());
				lightdag::uint128_t balance = node.ledger.account_balance (transaction, account);
				if (threshold.is_zero ())
				{
					boost::property_tree::ptree entry;
					lightdag::uint128_t pending = node.ledger.account_pending (transaction, account);
...
void lightdag::rpc_handler::wallet_change_seed ()
{
	if (rpc.config.enable_control)
	{
		std::string seed_text (request.get<std::string> ("seed"));
...
void lightdag::rpc_handler::wallet_contains ()
{
	std::string account_text (request.get<std::string> ("account"));
	std::string wallet_text (request.get<std::string> ("wallet"));
	lightdag::uint256_union account;
...
void lightdag::rpc_handler::wallet_create ()
{
	if (rpc.config.enable_control)
	{
		lightdag::keypair wallet_id;
...
void lightdag::rpc_handler::wallet_destroy ()
{
	if (rpc.config.enable_control)
	{
		std::string wallet_text (request.get<std::string> ("wallet"));
...
void lightdag::rpc_handler::wallet_export ()
{
	std::string wallet_text (request.get<std::string> ("wallet"));
	lightdag::uint256_union wallet;
	auto error (wallet.decode_hex (wallet_text));
...
void lightdag::rpc_handler::wallet_frontiers ()
{
	std::string wallet_text (request.get<std::string> ("wallet"));
	lightdag::uint256_union wallet;
	auto error (wallet.decode_hex (wallet_text));
...
void lightdag::rpc_handler::wallet_key_valid ()
{
	std::string wallet_text (request.get<std::string> ("wallet"));
	lightdag::uint256_union wallet;
	auto error (wallet.decode_hex (wallet_text));
...
void lightdag::rpc_handler::wallet_ledger ()
{
	const bool representative = request.get<bool> ("representative", false);
	const bool weight = request.get<bool> ("weight", false);
	const bool pending = request.get<bool> ("pending", false);
...
void lightdag::rpc_handler::wallet_lock ()
{
	if (rpc.config.enable_control)
	{
		std::string wallet_text (request.get<std::string> ("wallet"));
...
void lightdag::rpc_handler::wallet_pending ()
{
	std::string wallet_text (request.get<std::string> ("wallet"));
	lightdag::uint256_union wallet;
	auto error (wallet.decode_hex (wallet_text));
	if (!error)
	{
		...
		if (existing != node.wallets.items.end ())
		{
			...
			for (auto i (existing->second->store.begin (transaction)), n (existing->second->store.end ()); i != n; ++i)
			{
				...
				boost::property_tree::ptree peers_l;
				lightdag::account end (account.number () + 1);
				for (auto ii (node.store.pending_begin (transaction, lightdag::pending_key (account, 0))), nn (node.store.pending_begin (transaction, lightdag::pending_key (end, 0))); ii != nn && peers_l.size () < count; ++ii)
				{
					lightdag::pending_key key (ii->first);
					if (threshold.is_zero () && !source)
					{
						boost::property_tree::ptree entry;
						entry.put ("", key.hash.to_string ());
					...
					{
						lightdag::pending_info info (ii->second);
						if (info.amount.number () >= threshold.number ())
						{
							if (source)
							{
...
void lightdag::rpc_handler::wallet_representative ()
{
	std::string wallet_text (request.get<std::string> ("wallet"));
	lightdag::uint256_union wallet;
	auto error (wallet.decode_hex (wallet_text));
...
void lightdag::rpc_handler::wallet_representative_set ()
{
	if (rpc.config.enable_control)
	{
		std::string wallet_text (request.get<std::string> ("wallet"));
...
void lightdag::rpc_handler::wallet_republish ()
{
	if (rpc.config.enable_control)
	{
		std::string wallet_text (request.get<std::string> ("wallet"));
			...
			if (existing != node.wallets.items.end ())
			{
				...
				if (!error)
				{
					...
					for (auto i (existing->second->store.begin (transaction)), n (existing->second->store.end ()); i != n; ++i)
					{
						lightdag::account account (i->first.uint256 ());
						auto latest (node.ledger.latest (transaction, account));
						std::unique_ptr<lightdag::block> block;
						std::vector<lightdag::block_hash> hashes;
...
void lightdag::rpc_handler::wallet_work_get ()
{
	if (rpc.config.enable_control)
	{
		std::string wallet_text (request.get<std::string> ("wallet"));
		...
		if (!error)
		{
			...
			if (existing != node.wallets.items.end ())
			{
				...
				for (auto i (existing->second->store.begin (transaction)), n (existing->second->store.end ()); i != n; ++i)
				{
					...
					works.put (account.to_account (), lightdag::to_string_hex (work));
				}
...
void lightdag::rpc_handler::work_generate ()
{
	if (rpc.config.enable_control)
	{
		std::string hash_text (request.get<std::string> ("hash"));
		...
		if (!error)
		{
			...
			node.work.generate (hash, [rpc_l](boost::optional<uint64_t> const & work_a) {
				...
				{
					boost::property_tree::ptree response_l;
					response_l.put ("work", lightdag::to_string_hex (work_a.value ()));
...
void lightdag::rpc_handler::work_cancel ()
{
	if (rpc.config.enable_control)
	{
		std::string hash_text (request.get<std::string> ("hash"));
...
void lightdag::rpc_handler::work_get ()
{
	if (rpc.config.enable_control)
	{
		std::string wallet_text (request.get<std::string> ("wallet"));
			...
			if (existing != node.wallets.items.end ())
			{
				...
				if (!error)
				{
					...
					if (account_check != existing->second->store.end ())
					{
						...
						auto error_work (existing->second->store.work_get (transaction, account, work));
						boost::property_tree::ptree response_l;
						response_l.put ("work", lightdag::to_string_hex (work));
						response (response_l);
					}
					else
					{
...
void lightdag::rpc_handler::work_set ()
{
	if (rpc.config.enable_control)
	{
		std::string wallet_text (request.get<std::string> ("wallet"));
			...
			if (existing != node.wallets.items.end ())
			{
				...
				if (!error)
				{
					...
					if (account_check != existing->second->store.end ())
					{
						...
						uint64_t work;
						auto work_error (lightdag::from_string_hex (work_text, work));
						if (!work_error)
						{
							existing->second->store.work_put (transaction, account, work);
							boost::property_tree::ptree response_l;
...
void lightdag::rpc_handler::work_validate ()
{
	std::string hash_text (request.get<std::string> ("hash"));
	lightdag::block_hash hash;
	auto error (hash.decode_hex (hash_text));
	if (!error)
	{
		...
		if (!work_error)
		{
			auto validate (lightdag::work_validate (hash, work));
			boost::property_tree::ptree response_l;
...
void lightdag::rpc_handler::work_peer_add ()
{
	if (rpc.config.enable_control)
	{
		std::string address_text = request.get<std::string> ("address");
...
void lightdag::rpc_handler::work_peers ()
{
	if (rpc.config.enable_control)
	{
...
void lightdag::rpc_handler::work_peers_clear ()
{
	if (rpc.config.enable_control)
	{
...
lightdag::rpc_connection::rpc_connection (lightdag::node & node_a, lightdag::rpc & rpc_a) :
node (node_a.shared ()),
rpc (rpc_a),
...
void lightdag::rpc_connection::parse_connection ()
{
	read ();
}

void lightdag::rpc_connection::write_result (std::string body, unsigned version)
{
...
void lightdag::rpc_connection::read ()
{
	...
	boost::beast::http::async_read (socket, buffer, request, [this_l](boost::system::error_code const & ec, size_t bytes_transferred) {
		if (!ec)
		{
			this_l->node->background ([this_l]() {
				...
				{
					auto handler (std::make_shared<lightdag::rpc_handler> (*this_l->node, this_l->rpc, this_l->request.body (), response_handler));
...
void lightdag::rpc_handler::process_request ()
{
	try
	{
		...
		else if (action == "klightdag_from_raw")
		{
			klightdag_from_raw ();
		}
		else if (action == "klightdag_to_raw")
		{
			klightdag_to_raw ();
		}
		...
		else if (action == "mlightdag_from_raw")
		{
			mlightdag_from_raw ();
		}
		else if (action == "mlightdag_to_raw")
		{
			mlightdag_to_raw ();
		}
		else if (action == "password_change")
		...
		else if (action == "lightdag_from_raw")
		{
			lightdag_from_raw ();
		}
		else if (action == "lightdag_to_raw")
		{
			lightdag_to_raw ();
		}
...
lightdag::payment_observer::payment_observer (std::function<void(boost::property_tree::ptree const &)> const & response_a, lightdag::rpc & rpc_a, lightdag::account const & account_a, lightdag::amount const & amount_a) :
rpc (rpc_a),
account (account_a),
amount (amount_a),
response (response_a)
...
void lightdag::payment_observer::start (uint64_t timeout)
{
	auto this_l (shared_from_this ());
	rpc.node.alarm.add (std::chrono::steady_clock::now () + std::chrono::milliseconds (timeout), [this_l]() {
		this_l->complete (lightdag::payment_status::nothing);
	});
}

lightdag::payment_observer::~payment_observer ()
{
}

void lightdag::payment_observer::observe ()
{
	if (rpc.node.balance (account) >= amount.number ())
	{
		complete (lightdag::payment_status::success);
	}
}

void lightdag::payment_observer::complete (lightdag::payment_status status)
{
	...
	if (!already)
	{
		...
		switch (status)
		{
			case lightdag::payment_status::nothing:
			{
				boost::property_tree::ptree response_l;
			...
			case lightdag::payment_status::success:
			{
				boost::property_tree::ptree response_l;
...
std::unique_ptr<lightdag::rpc> lightdag::get_rpc (boost::asio::io_service & service_a, lightdag::node & node_a, lightdag::rpc_config const & config_a)
{
