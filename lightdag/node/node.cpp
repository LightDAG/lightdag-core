...
double constexpr lightdag::node::price_max;
double constexpr lightdag::node::free_cutoff;
std::chrono::seconds constexpr lightdag::node::period;
std::chrono::seconds constexpr lightdag::node::cutoff;
std::chrono::minutes constexpr lightdag::node::backup_interval;
int constexpr lightdag::port_mapping::mapping_timeout;
int constexpr lightdag::port_mapping::check_timeout;
unsigned constexpr lightdag::active_transactions::announce_interval_ms;

lightdag::message_statistics::message_statistics () :
keepalive (0),
publish (0),
confirm_req (0),
confirm_ack (0)
...
lightdag::network::network (lightdag::node & node_a, uint16_t port) :
socket (node_a.service, lightdag::endpoint (boost::asio::ip::address_v6::any (), port)),
...
void lightdag::network::send_keepalive (lightdag::endpoint const & endpoint_a)
{
	assert (endpoint_a.address ().is_v6 ());
	lightdag::keepalive message;
	node.peers.random_fill (message.peers);
	std::shared_ptr<std::vector<uint8_t>> bytes (new std::vector<uint8_t>);
	{
		lightdag::vectorstream stream (*bytes);
	...
	++outgoing.keepalive;
	std::weak_ptr<lightdag::node> node_w (node.shared ());
...
void lightdag::node::keepalive (std::string const & address_a, uint16_t port_a)
{
	...
	network.resolver.async_resolve (boost::asio::ip::udp::resolver::query (address_a, std::to_string (port_a)), [node_l, address_a, port_a](boost::system::error_code const & ec, boost::asio::ip::udp::resolver::iterator i_a) {
		if (!ec)
		{
			for (auto i (i_a), n (boost::asio::ip::udp::resolver::iterator{}); i != n; ++i)
			{
				...
				{
					endpoint = lightdag::endpoint (boost::asio::ip::address_v6::v4_mapped (endpoint.address ().to_v4 ()), endpoint.port ());
				}
...
void lightdag::network::republish (lightdag::block_hash const & hash_a, std::shared_ptr<std::vector<uint8_t>> buffer_a, lightdag::endpoint endpoint_a)
{
	++outgoing.publish;
...
void lightdag::network::rebroadcast_reps (std::shared_ptr<lightdag::block> block_a)
{
	auto hash (block_a->hash ());
	lightdag::publish message (block_a);
	std::shared_ptr<std::vector<uint8_t>> bytes (new std::vector<uint8_t>);
	{
		lightdag::vectorstream stream (*bytes);
		message.serialize (stream);
...
template <typename T>
bool confirm_block (MDB_txn * transaction_a, lightdag::node & node_a, T & list_a, std::shared_ptr<lightdag::block> block_a)
{
	bool result (false);
	if (node_a.config.enable_voting)
	{
		node_a.wallets.foreach_representative (transaction_a, [&result, &block_a, &list_a, &node_a, &transaction_a](lightdag::public_key const & pub_a, lightdag::raw_key const & prv_a) {
			result = true;
			auto vote (node_a.store.vote_generate (transaction_a, pub_a, prv_a, block_a));
			lightdag::confirm_ack confirm (vote);
			std::shared_ptr<std::vector<uint8_t>> bytes (new std::vector<uint8_t>);
			{
				lightdag::vectorstream stream (*bytes);
...
template <>
bool confirm_block (MDB_txn * transaction_a, lightdag::node & node_a, lightdag::endpoint & peer_a, std::shared_ptr<lightdag::block> block_a)
{
	std::array<lightdag::endpoint, 1> endpoints;
	endpoints[0] = peer_a;
	auto result (confirm_block (transaction_a, node_a, endpoints, std::move (block_a)));
	return result;
}

void lightdag::network::republish_block (MDB_txn * transaction, std::shared_ptr<lightdag::block> block)
{
	...
	{
		lightdag::publish message (block);
		std::shared_ptr<std::vector<uint8_t>> bytes (new std::vector<uint8_t>);
		{
			lightdag::vectorstream stream (*bytes);
			message.serialize (stream);
...
void lightdag::network::republish_vote (std::shared_ptr<lightdag::vote> vote_a)
{
	lightdag::confirm_ack confirm (vote_a);
	std::shared_ptr<std::vector<uint8_t>> bytes (new std::vector<uint8_t>);
	{
		lightdag::vectorstream stream (*bytes);
		confirm.serialize (stream);
...
void lightdag::network::broadcast_confirm_req (std::shared_ptr<lightdag::block> block_a)
{
...
void lightdag::network::send_confirm_req (lightdag::endpoint const & endpoint_a, std::shared_ptr<lightdag::block> block)
{
	lightdag::confirm_req message (block);
	std::shared_ptr<std::vector<uint8_t>> bytes (new std::vector<uint8_t>);
	{
		lightdag::vectorstream stream (*bytes);
		message.serialize (stream);
...
template <typename T>
void rep_query (lightdag::node & node_a, T const & peers_a)
{
	lightdag::transaction transaction (node_a.store.environment, nullptr, false);
	std::shared_ptr<lightdag::block> block (node_a.store.block_random (transaction));
	auto hash (block->hash ());
	...
	std::weak_ptr<lightdag::node> node_w (node_a.shared ());
...
template <>
void rep_query (lightdag::node & node_a, lightdag::endpoint const & peers_a)
{
	std::array<lightdag::endpoint, 1> peers;
	peers[0] = peers_a;
...
namespace
{
class network_message_visitor : public lightdag::message_visitor
{
public:
	network_message_visitor (lightdag::node & node_a, lightdag::endpoint const & sender_a) :
	node (node_a),
	sender (sender_a)
	{
	}
	virtual ~network_message_visitor () = default;
	void keepalive (lightdag::keepalive const & message_a) override
	{
		if (node.config.logging.network_keepalive_logging ())
		{
	...
	void publish (lightdag::publish const & message_a) override
	{
		if (node.config.logging.network_message_logging ())
		{
	...
	void confirm_req (lightdag::confirm_req const & message_a) override
	{
		if (node.config.logging.network_message_logging ())
		{
		...
		node.peers.insert (sender, message_a.version_using);
		node.process_active (message_a.block);
		lightdag::transaction transaction_a (node.store.environment, nullptr, false);
		if (node.store.block_exists (transaction_a, message_a.block->hash ()))
		{
			confirm_block (transaction_a, node, sender, message_a.block);
		}
	}
	void confirm_ack (lightdag::confirm_ack const & message_a) override
	{
		if (node.config.logging.network_message_logging ())
		{
		...
		if (vote.code == lightdag::vote_code::replay)
		{
			...
			{
				lightdag::confirm_ack confirm (vote.vote);
				std::shared_ptr<std::vector<uint8_t>> bytes (new std::vector<uint8_t>);
				{
					lightdag::vectorstream stream (*bytes);
					confirm.serialize (stream);
	...
	void bulk_pull (lightdag::bulk_pull const &) override
	{
		assert (false);
	}
	void bulk_pull_blocks (lightdag::bulk_pull_blocks const &) override
	{
		assert (false);
	}
	void bulk_push (lightdag::bulk_push const &) override
	{
		assert (false);
	}
	void frontier_req (lightdag::frontier_req const &) override
	{
		assert (false);
	}
	lightdag::node & node;
	lightdag::endpoint sender;
};
}

void lightdag::network::receive_action (boost::system::error_code const & error, size_t size_a)
{
	if (!error && on)
	{
		if (!lightdag::reserved_address (remote) && remote != endpoint ())
		{
			network_message_visitor visitor (node, remote);
			lightdag::message_parser parser (visitor, node.work);
			parser.deserialize_buffer (buffer.data (), size_a);
			if (parser.status != lightdag::message_parser::parse_status::success)
			{
				++error_count;

				if (parser.status == lightdag::message_parser::parse_status::insufficient_work)
				{
				...
				else if (parser.status == lightdag::message_parser::parse_status::invalid_message_type)
				{
					if (node.config.logging.network_logging ())
					{
				...
				else if (parser.status == lightdag::message_parser::parse_status::invalid_header)
				{
					if (node.config.logging.network_logging ())
					{
				...
				else if (parser.status == lightdag::message_parser::parse_status::invalid_keepalive_message)
				{
					if (node.config.logging.network_logging ())
					{
				...
				else if (parser.status == lightdag::message_parser::parse_status::invalid_publish_message)
				{
					if (node.config.logging.network_logging ())
					{
				...
				else if (parser.status == lightdag::message_parser::parse_status::invalid_confirm_req_message)
				{
					if (node.config.logging.network_logging ())
					{
				...
				else if (parser.status == lightdag::message_parser::parse_status::invalid_confirm_ack_message)
				{
					if (node.config.logging.network_logging ())
					{
...
void lightdag::network::merge_peers (std::array<lightdag::endpoint, 8> const & peers_a)
{
...
bool lightdag::operation::operator> (lightdag::operation const & other_a) const
{
	return wakeup > other_a.wakeup;
}

lightdag::alarm::alarm (boost::asio::io_service & service_a) :
service (service_a),
thread ([this]() { run (); })
{
}
...
void lightdag::alarm::add (std::chrono::steady_clock::time_point const & wakeup_a, std::function<void()> const & operation)
{
	std::lock_guard<std::mutex> lock (mutex);
	operations.push (lightdag::operation ({ wakeup_a, operation }));
...
void lightdag::logging::init (boost::filesystem::path const & application_path_a)
{
...
void lightdag::logging::serialize_json (boost::property_tree::ptree & tree_a) const
{
...
bool lightdag::logging::upgrade_json (unsigned version_a, boost::property_tree::ptree & tree_a)
{
	auto result (false);
	switch (version_a)
	{
...
bool lightdag::logging::deserialize_json (bool & upgraded_a, boost::property_tree::ptree & tree_a)
{
	auto result (false);
	try
	{
...
bool lightdag::logging::ledger_logging () const
{
	return ledger_logging_value;
}

bool lightdag::logging::ledger_duplicate_logging () const
{
	return ledger_logging () && ledger_duplicate_logging_value;
}

bool lightdag::logging::vote_logging () const
{
	return vote_logging_value;
}

bool lightdag::logging::network_logging () const
{
	return network_logging_value;
}

bool lightdag::logging::network_message_logging () const
{
	return network_logging () && network_message_logging_value;
}

bool lightdag::logging::network_publish_logging () const
{
	return network_logging () && network_publish_logging_value;
}

bool lightdag::logging::network_packet_logging () const
{
	return network_logging () && network_packet_logging_value;
}

bool lightdag::logging::network_keepalive_logging () const
{
	return network_logging () && network_keepalive_logging_value;
}

bool lightdag::logging::node_lifetime_tracing () const
{
	return node_lifetime_tracing_value;
}

bool lightdag::logging::insufficient_work_logging () const
{
	return network_logging () && insufficient_work_logging_value;
}

bool lightdag::logging::log_rpc () const
{
	return network_logging () && log_rpc_value;
}

bool lightdag::logging::bulk_pull_logging () const
{
	return network_logging () && bulk_pull_logging_value;
}

bool lightdag::logging::callback_logging () const
{
	return network_logging ();
}

bool lightdag::logging::work_generation_time () const
{
	return work_generation_time_value;
}

bool lightdag::logging::log_to_cerr () const
{
	return log_to_cerr_value;
}

lightdag::node_init::node_init () :
block_store_init (false),
wallet_init (false)
{
}

bool lightdag::node_init::error ()
{
	return block_store_init || wallet_init;
}

lightdag::node_config::node_config () :
node_config (lightdag::network::node_port, lightdag::logging ())
{
}

lightdag::node_config::node_config (uint16_t peering_port_a, lightdag::logging const & logging_a) :
peering_port (peering_port_a),
logging (logging_a),
bootstrap_fraction_numerator (1),
receive_minimum (lightdag::ldg_ratio),
inactive_supply (0),
...
state_block_generate_canary (0)
{
	switch (lightdag::lightdag_network)
	{
		case lightdag::lightdag_networks::lightdag_test_network:
			preconfigured_representatives.push_back (lightdag::genesis_account);
			break;
		case lightdag::lightdag_networks::lightdag_beta_network:
			preconfigured_peers.push_back ("beta.lightdag.net");
			preconfigured_representatives.push_back (lightdag::account ("C93F714298E6061E549E52BB8885085319BE977B3FE8F03A1B726E9BE4BE38DE"));
			state_block_parse_canary = lightdag::block_hash ("5005F5283DE8D2DAB0DAC41DE9BD23640F962B4F0EA7D3128C2EA3D78D578E27");
			state_block_generate_canary = lightdag::block_hash ("FC18E2265FB835E8CF60E63531053A768CEDF5194263B01A5C95574944E4660D");
			break;
		case lightdag::lightdag_networks::lightdag_live_network:
			preconfigured_peers.push_back ("live.lightdag.net");
			preconfigured_representatives.push_back (lightdag::account ("A30E0A32ED41C8607AA9212843392E853FCBCB4E7CB194E35C94F07F91DE59EF"));
			preconfigured_representatives.push_back (lightdag::account ("67556D31DDFC2A440BF6147501449B4CB9572278D034EE686A6BEE29851681DF"));
			preconfigured_representatives.push_back (lightdag::account ("5C2FBB148E006A8E8BA7A75DD86C9FE00C83F5FFDBFD76EAA09531071436B6AF"));
			preconfigured_representatives.push_back (lightdag::account ("AE7AC63990DAAAF2A69BF11C913B928844BF5012355456F2F164166464024B29"));
			preconfigured_representatives.push_back (lightdag::account ("BD6267D6ECD8038327D2BCC0850BDF8F56EC0414912207E81BCF90DFAC8A4AAA"));
			preconfigured_representatives.push_back (lightdag::account ("2399A083C600AA0572F5E36247D978FCFC840405F8D4B6D33161C0066A55F431"));
			preconfigured_representatives.push_back (lightdag::account ("2298FAB7C61058E77EA554CB93EDEEDA0692CBFCC540AB213B2836B29029E23A"));
			preconfigured_representatives.push_back (lightdag::account ("3FE80B4BC842E82C1C18ABFEEC47EA989E63953BC82AC411F304D13833D52A56"));
			state_block_parse_canary = lightdag::block_hash ("89F1C0AC4C5AD23964AB880571E3EA67FDC41BD11AB20E67F0A29CF94CD4E24A");
			state_block_generate_canary = lightdag::block_hash ("B6DC4D64801BEC7D81DAA086A5733D251E8CBA0E9226FD6173D97C0569EC2998");
			break;
		default:
			assert (false);
			break;
...
void lightdag::node_config::serialize_json (boost::property_tree::ptree & tree_a) const
{
...
bool lightdag::node_config::upgrade_json (unsigned version, boost::property_tree::ptree & tree_a)
{
	auto result (false);
	switch (version)
	{
		case 1:
		{
			...
			{
				lightdag::uint256_union account;
		...
		case 2:
		{
			tree_a.put ("inactive_supply", lightdag::uint128_union (0).to_string_dec ());
		...
		case 3:
			...
			tree_a.put ("receive_minimum", lightdag::ldg_ratio.convert_to<std::string> ());
			tree_a.erase ("version");
...
bool lightdag::node_config::deserialize_json (bool & upgraded_a, boost::property_tree::ptree & tree_a)
{
	auto result (false);
	try
	{
		...
		for (auto i (work_peers_l.begin ()), n (work_peers_l.end ()); i != n; ++i)
		{
			...
			uint16_t port;
			result |= lightdag::parse_address_port (work_peer, address, port);
			if (!result)
			{
		...
		{
			lightdag::account representative (0);
...
lightdag::account lightdag::node_config::random_representative ()
{
	assert (preconfigured_representatives.size () > 0);
	size_t index (lightdag::random_pool.GenerateWord32 (0, preconfigured_representatives.size () - 1));
	auto result (preconfigured_representatives[index]);
	return result;
}

lightdag::vote_processor::vote_processor (lightdag::node & node_a) :
node (node_a)
{
}

lightdag::vote_result lightdag::vote_processor::vote (std::shared_ptr<lightdag::vote> vote_a, lightdag::endpoint endpoint_a)
{
	lightdag::vote_result result = { lightdag::vote_code::invalid, vote_a };
	if (!lightdag::validate_message (vote_a->account, vote_a->hash (), vote_a->signature))
	{
		result.code = lightdag::vote_code::replay;
		std::shared_ptr<lightdag::vote> newest_vote;
		{
			lightdag::transaction transaction (node.store.environment, nullptr, false);
			newest_vote = node.store.vote_max (transaction, vote_a);
		}
		if (!node.active.vote (vote_a))
		{
			result.code = lightdag::vote_code::vote;
		}
		else
		{
	...
	if (node.config.logging.vote_logging ())
	{
		...
		switch (result.code)
		{
			case lightdag::vote_code::invalid:
				status = "Invalid";
				break;
			case lightdag::vote_code::replay:
				status = "Replay";
				break;
			case lightdag::vote_code::vote:
				status = "Vote";
				break;
		}
...
void lightdag::rep_crawler::add (lightdag::block_hash const & hash_a)
{
	std::lock_guard<std::mutex> lock (mutex);
	active.insert (hash_a);
}

void lightdag::rep_crawler::remove (lightdag::block_hash const & hash_a)
{
	std::lock_guard<std::mutex> lock (mutex);
	active.erase (hash_a);
}

bool lightdag::rep_crawler::exists (lightdag::block_hash const & hash_a)
{
	std::lock_guard<std::mutex> lock (mutex);
	return active.count (hash_a) != 0;
}

lightdag::block_processor_item::block_processor_item (std::shared_ptr<lightdag::block> block_a) :
block_processor_item (block_a, false)
{
}

lightdag::block_processor_item::block_processor_item (std::shared_ptr<lightdag::block> block_a, bool force_a) :
block (block_a),
force (force_a)
{
}

lightdag::block_processor::block_processor (lightdag::node & node_a) :
stopped (false),
idle (true),
node (node_a)
{
...
void lightdag::block_processor::stop ()
{
	std::lock_guard<std::mutex> lock (mutex);
	stopped = true;
	condition.notify_all ();
...
void lightdag::block_processor::flush ()
{
	std::unique_lock<std::mutex> lock (mutex);
...
void lightdag::block_processor::add (lightdag::block_processor_item const & item_a)
{
	std::lock_guard<std::mutex> lock (mutex);
...
void lightdag::block_processor::process_blocks ()
{
	std::unique_lock<std::mutex> lock (mutex);
	while (!stopped)
	{
		if (!blocks.empty ())
		{
			std::deque<lightdag::block_processor_item> blocks_processing;
...
void lightdag::block_processor::process_receive_many (lightdag::block_processor_item const & item_a)
{
	std::deque<lightdag::block_processor_item> blocks_processing;
...
void lightdag::block_processor::process_receive_many (std::deque<lightdag::block_processor_item> & blocks_processing)
{
	while (!blocks_processing.empty ())
	{
		std::deque<std::pair<std::shared_ptr<lightdag::block>, lightdag::process_return>> progress;
		{
			lightdag::transaction transaction (node.store.environment, nullptr, true);
			auto cutoff (std::chrono::steady_clock::now () + lightdag::transaction_timeout);
			while (!blocks_processing.empty () && std::chrono::steady_clock::now () < cutoff)
			{
				...
				{
					case lightdag::process_result::progress:
					{
						progress.push_back (std::make_pair (item.block, process_result));
					}
					case lightdag::process_result::old:
					{
						...
						for (auto i (cached.begin ()), n (cached.end ()); i != n; ++i)
						{
							...
							blocks_processing.push_front (lightdag::block_processor_item (*i));
						}
...
lightdag::process_return lightdag::block_processor::process_receive_one (MDB_txn * transaction_a, std::shared_ptr<lightdag::block> block_a)
{
	...
	switch (result.code)
	{
		...
		case lightdag::process_result::gap_previous:
		{
			if (node.config.logging.ledger_logging ())
			{
		...
		case lightdag::process_result::gap_source:
		{
			if (node.config.logging.ledger_logging ())
			{
		...
		case lightdag::process_result::state_block_disabled:
		{
			if (node.config.logging.ledger_logging ())
			{
		...
		case lightdag::process_result::bad_signature:
		{
			if (node.config.logging.ledger_logging ())
			{
		...
		case lightdag::process_result::negative_spend:
		{
			if (node.config.logging.ledger_logging ())
			{
		...
		case lightdag::process_result::unreceivable:
		{
			if (node.config.logging.ledger_logging ())
			{
		...
		case lightdag::process_result::not_receive_from_send:
		{
			if (node.config.logging.ledger_logging ())
			{
		...
		case lightdag::process_result::fork:
		{
		...
		case lightdag::process_result::account_mismatch:
		{
			if (node.config.logging.ledger_logging ())
			{
		...
		case lightdag::process_result::block_position:
		{
			if (node.config.logging.ledger_logging ())
			{
...
lightdag::node::node (lightdag::node_init & init_a, boost::asio::io_service & service_a, uint16_t peering_port_a, boost::filesystem::path const & application_path_a, lightdag::alarm & alarm_a, lightdag::logging const & logging_a, lightdag::work_pool & work_a) :
node (init_a, service_a, application_path_a, alarm_a, lightdag::node_config (peering_port_a, logging_a), work_a)
{
}

lightdag::node::node (lightdag::node_init & init_a, boost::asio::io_service & service_a, boost::filesystem::path const & application_path_a, lightdag::alarm & alarm_a, lightdag::node_config const & config_a, lightdag::work_pool & work_a) :
service (service_a),
config (config_a),
alarm (alarm_a),
work (work_a),
...
block_processor_thread ([this]() { this->block_processor.process_blocks (); })
{
	...
	peers.peer_observer = [this](lightdag::endpoint const & endpoint_a) {
		observers.endpoint (endpoint_a);
	};
	peers.disconnect_observer = [this]() {
		observers.disconnect ();
	};
	observers.blocks.add ([this](std::shared_ptr<lightdag::block> block_a, lightdag::process_return const & result_a) {
	...
	observers.endpoint.add ([this](lightdag::endpoint const & endpoint_a) {
		this->network.send_keepalive (endpoint_a);
		rep_query (*this, endpoint_a);
	});
	observers.vote.add ([this](std::shared_ptr<lightdag::vote> vote_a, lightdag::endpoint const &) {
		this->gap_cache.vote (vote_a);
	});
	observers.vote.add ([this](std::shared_ptr<lightdag::vote> vote_a, lightdag::endpoint const & endpoint_a) {
	...
	BOOST_LOG (log) << "Node starting, version: " << LIGHTDAG_VERSION_MAJOR << "." << LIGHTDAG_VERSION_MINOR;
	...
	if (lightdag::lightdag_network == lightdag::lightdag_networks::lightdag_live_network)
	{
		extern const char lightdag_bootstrap_weights[];
		extern const size_t lightdag_bootstrap_weights_size;
		lightdag::bufferstream weight_stream ((const uint8_t *)lightdag_bootstrap_weights, lightdag_bootstrap_weights_size);
		lightdag::uint128_union block_height;
		if (!lightdag::read (weight_stream, block_height))
		{
			...
			if (ledger.store.block_count (transaction).sum () < max_blocks)
			{
				...
				while (true)
				{
					...
					if (lightdag::read (weight_stream, account.bytes))
					{
						break;
					}
					lightdag::amount weight;
					if (lightdag::read (weight_stream, weight.bytes))
					{
						break;
					}
					BOOST_LOG (log) << "Using bootstrap rep weight: " << account.to_account () << " -> " << weight.format_balance (Mldg_ratio, 0, true) << " LDG";
...
bool lightdag::node::copy_with_compaction (boost::filesystem::path const & destination_file)
{
	return !mdb_env_copy2 (store.environment.environment,
	destination_file.string ().c_str (), MDB_CP_COMPACT);
}

void lightdag::node::send_keepalive (lightdag::endpoint const & endpoint_a)
{
	auto endpoint_l (endpoint_a);
	if (endpoint_l.address ().is_v4 ())
	{
		endpoint_l = lightdag::endpoint (boost::asio::ip::address_v6::v4_mapped (endpoint_l.address ().to_v4 ()), endpoint_l.port ());
	}
	assert (endpoint_l.address ().is_v6 ());
	network.send_keepalive (endpoint_l);
}

lightdag::gap_cache::gap_cache (lightdag::node & node_a) :
node (node_a)
{
}

void lightdag::gap_cache::add (MDB_txn * transaction_a, std::shared_ptr<lightdag::block> block_a)
{
	auto hash (block_a->hash ());
	std::lock_guard<std::mutex> lock (mutex);
	...
	{
		blocks.get<1> ().modify (existing, [](lightdag::gap_information & info) {
			info.arrival = std::chrono::steady_clock::now ();
		});
	}
	else
	{
		blocks.insert ({ std::chrono::steady_clock::now (), hash, std::unique_ptr<lightdag::votes> (new lightdag::votes (block_a)) });
		if (blocks.size () > max)
		{
...
void lightdag::gap_cache::vote (std::shared_ptr<lightdag::vote> vote_a)
{
	lightdag::transaction transaction (node.store.environment, nullptr, false);
	std::lock_guard<std::mutex> lock (mutex);
	...
	if (existing != blocks.get<1> ().end ())
	{
		...
		if (winner.first > bootstrap_threshold (transaction))
		{
			...
			node.alarm.add (lightdag::lightdag_network == lightdag::lightdag_networks::lightdag_test_network ? now + std::chrono::milliseconds (5) : now + std::chrono::seconds (5), [node_l, hash]() {
				lightdag::transaction transaction (node_l->store.environment, nullptr, false);
...
lightdag::uint128_t lightdag::gap_cache::bootstrap_threshold (MDB_txn * transaction_a)
{
	auto result ((node.ledger.supply (transaction_a) / 256) * node.config.bootstrap_fraction_numerator);
	return result;
}

void lightdag::gap_cache::purge_old ()
{
...
void lightdag::network::confirm_send (lightdag::confirm_ack const & confirm_a, std::shared_ptr<std::vector<uint8_t>> bytes_a, lightdag::endpoint const & endpoint_a)
{
	if (node.config.logging.network_publish_logging ())
	{
...
void lightdag::node::process_active (std::shared_ptr<lightdag::block> incoming)
{
	block_arrival.add (incoming->hash ());
	block_processor.add (incoming);
}

lightdag::process_return lightdag::node::process (lightdag::block const & block_a)
{
	lightdag::transaction transaction (store.environment, nullptr, true);
...
std::vector<lightdag::endpoint> lightdag::peer_container::list_sqrt ()
{
	auto peers (random_set (2 * size_sqrt ()));
	std::vector<lightdag::endpoint> result;
...
std::vector<lightdag::endpoint> lightdag::peer_container::list ()
{
	std::vector<lightdag::endpoint> result;
	std::lock_guard<std::mutex> lock (mutex);
	result.reserve (peers.size ());
...
std::map<lightdag::endpoint, unsigned> lightdag::peer_container::list_version ()
{
	std::map<lightdag::endpoint, unsigned> result;
	std::lock_guard<std::mutex> lock (mutex);
	for (auto i (peers.begin ()), j (peers.end ()); i != j; ++i)
	{
		result.insert (std::pair<lightdag::endpoint, unsigned> (i->endpoint, i->network_version));
	}
	return result;
}

lightdag::endpoint lightdag::peer_container::bootstrap_peer ()
{
	lightdag::endpoint result (boost::asio::ip::address_v6::any (), 0);
	std::lock_guard<std::mutex> lock (mutex);
	;
	for (auto i (peers.get<4> ().begin ()), n (peers.get<4> ().end ()); i != n;)
	{
		...
		{
			result = i->endpoint;
			peers.get<4> ().modify (i, [](lightdag::peer_information & peer_a) {
...
bool lightdag::parse_port (std::string const & string_a, uint16_t & port_a)
{
	bool result;
	size_t converted;
...
bool lightdag::parse_address_port (std::string const & string, boost::asio::ip::address & address_a, uint16_t & port_a)
{
	auto result (false);
...
bool lightdag::parse_endpoint (std::string const & string, lightdag::endpoint & endpoint_a)
{
	boost::asio::ip::address address;
	uint16_t port;
	...
	if (!result)
	{
		endpoint_a = lightdag::endpoint (address, port);
	}
	return result;
}

bool lightdag::parse_tcp_endpoint (std::string const & string, lightdag::tcp_endpoint & endpoint_a)
{
	boost::asio::ip::address address;
	uint16_t port;
	auto result (parse_address_port (string, address, port));
	if (!result)
	{
		endpoint_a = lightdag::tcp_endpoint (address, port);
	}
	return result;
}

void lightdag::node::start ()
{
	network.receive ();
	ongoing_keepalive ();
	ongoing_bootstrap ();
...
void lightdag::node::keepalive_preconfigured (std::vector<std::string> const & peers_a)
{
	for (auto i (peers_a.begin ()), n (peers_a.end ()); i != n; ++i)
	{
		keepalive (*i, lightdag::network::node_port);
	}
}

lightdag::block_hash lightdag::node::latest (lightdag::account const & account_a)
{
	lightdag::transaction transaction (store.environment, nullptr, false);
	return ledger.latest (transaction, account_a);
}

lightdag::uint128_t lightdag::node::balance (lightdag::account const & account_a)
{
	lightdag::transaction transaction (store.environment, nullptr, false);
	return ledger.account_balance (transaction, account_a);
}

std::unique_ptr<lightdag::block> lightdag::node::block (lightdag::block_hash const & hash_a)
{
	lightdag::transaction transaction (store.environment, nullptr, false);
	return store.block_get (transaction, hash_a);
}

std::pair<lightdag::uint128_t, lightdag::uint128_t> lightdag::node::balance_pending (lightdag::account const & account_a)
{
	std::pair<lightdag::uint128_t, lightdag::uint128_t> result;
	lightdag::transaction transaction (store.environment, nullptr, false);
...
lightdag::uint128_t lightdag::node::weight (lightdag::account const & account_a)
{
	lightdag::transaction transaction (store.environment, nullptr, false);
	return ledger.weight (transaction, account_a);
}

lightdag::account lightdag::node::representative (lightdag::account const & account_a)
{
	lightdag::transaction transaction (store.environment, nullptr, false);
	lightdag::account_info info;
	lightdag::account result (0);
...
void lightdag::node::ongoing_keepalive ()
{
	...
	std::weak_ptr<lightdag::node> node_w (shared_from_this ());
...
void lightdag::node::ongoing_rep_crawl ()
{
	auto now (std::chrono::steady_clock::now ());
...
void lightdag::node::ongoing_bootstrap ()
{
...
void lightdag::node::ongoing_store_flush ()
{
	{
		lightdag::transaction transaction (store.environment, nullptr, true);
...
void lightdag::node::backup_wallet ()
{
	lightdag::transaction transaction (store.environment, nullptr, false);
...
int lightdag::node::price (lightdag::uint128_t const & balance_a, int amount_a)
{
	assert (balance_a >= amount_a * lightdag::Gldg_ratio);
	auto balance_l (balance_a);
	double result (0.0);
	for (auto i (0); i < amount_a; ++i)
	{
		balance_l -= lightdag::Gldg_ratio;
		auto balance_scaled ((balance_l / lightdag::Mldg_ratio).convert_to<double> ());
...
{
public:
	distributed_work (std::shared_ptr<lightdag::node> const & node_a, lightdag::block_hash const & root_a, std::function<void(uint64_t)> callback_a) :
	callback (callback_a),
	node (node_a),
	root (root_a)
	{
		...
		if (!outstanding.empty ())
		{
			...
			for (auto const & i : outstanding)
			{
				...
				node->background ([this_l, host, service]() {
					...
					connection->socket.async_connect (lightdag::tcp_endpoint (host, service), [this_l, connection](boost::system::error_code const & ec) {
						if (!ec)
						{
	...
	void success (std::string const & body_a, boost::asio::ip::address const & address)
	{
		...
		try
		{
			...
			uint64_t work;
			if (!lightdag::from_string_hex (work_text, work))
			{
...
void lightdag::node::generate_work (lightdag::block & block_a)
{
	block_a.block_work_set (generate_work (block_a.root ()));
}

void lightdag::node::generate_work (lightdag::uint256_union const & hash_a, std::function<void(uint64_t)> callback_a)
{
	auto work_generation (std::make_shared<distributed_work> (shared (), hash_a, callback_a));
	work_generation->start ();
}

uint64_t lightdag::node::generate_work (lightdag::uint256_union const & hash_a)
{
...
void lightdag::node::add_initial_peers ()
{
}

namespace
{
class confirmed_visitor : public lightdag::block_visitor
{
public:
	confirmed_visitor (lightdag::node & node_a, std::shared_ptr<lightdag::block> block_a) :
	node (node_a),
	block (block_a)
	{
	}
	virtual ~confirmed_visitor () = default;
	void scan_receivable (lightdag::account const & account_a)
	{
		for (auto i (node.wallets.items.begin ()), n (node.wallets.items.end ()); i != n; ++i)
		{
			...
			if (wallet->exists (account_a))
			{
				...
				auto error (node.store.pending_get (transaction, lightdag::pending_key (account_a, block->hash ()), pending));
				if (!error)
				{
					auto node_l (node.shared ());
					auto amount (pending.amount.number ());
					wallet->receive_async (block, representative, amount, [](std::shared_ptr<lightdag::block>) {});
				}
				else
				{
					if (node.config.logging.ledger_duplicate_logging ())
...
void lightdag::node::process_confirmed (std::shared_ptr<lightdag::block> confirmed_a)
{
	confirmed_visitor visitor (*this, confirmed_a);
	confirmed_a->visit (visitor);
}

void lightdag::node::process_message (lightdag::message & message_a, lightdag::endpoint const & sender_a)
{
	network_message_visitor visitor (*this, sender_a);
	message_a.visit (visitor);
}

lightdag::endpoint lightdag::network::endpoint ()
{
	boost::system::error_code ec;
	...
	return lightdag::endpoint (boost::asio::ip::address_v6::loopback (), port);
}

void lightdag::block_arrival::add (lightdag::block_hash const & hash_a)
{
	std::lock_guard<std::mutex> lock (mutex);
	auto now (std::chrono::steady_clock::now ());
	arrival.insert (lightdag::block_arrival_info{ now, hash_a });
}

bool lightdag::block_arrival::recent (lightdag::block_hash const & hash_a)
{
	std::lock_guard<std::mutex> lock (mutex);
	auto now (std::chrono::steady_clock::now ());
...
std::unordered_set<lightdag::endpoint> lightdag::peer_container::random_set (size_t count_a)
{
	std::unordered_set<lightdag::endpoint> result;
...
void lightdag::peer_container::random_fill (std::array<lightdag::endpoint, 8> & target_a)
{
	auto peers (random_set (target_a.size ()));
	assert (peers.size () <= target_a.size ());
	auto endpoint (lightdag::endpoint (boost::asio::ip::address_v6{}, 0));
...
std::vector<lightdag::peer_information> lightdag::peer_container::representatives (size_t count_a)
{
...
std::vector<lightdag::peer_information> lightdag::peer_container::purge_list (std::chrono::steady_clock::time_point const & cutoff)
{
	std::vector<lightdag::peer_information> result;
	{
		std::lock_guard<std::mutex> lock (mutex);
		...
		for (auto i (peers.begin ()), n (peers.end ()); i != n; ++i)
		{
			peers.modify (i, [](lightdag::peer_information & info) { info.last_attempt = std::chrono::steady_clock::now (); });
		}
...
std::vector<lightdag::endpoint> lightdag::peer_container::rep_crawl ()
{
	std::vector<lightdag::endpoint> result;
	result.reserve (8);
	std::lock_guard<std::mutex> lock (mutex);
...
size_t lightdag::peer_container::size ()
{
	std::lock_guard<std::mutex> lock (mutex);
	return peers.size ();
}

size_t lightdag::peer_container::size_sqrt ()
{
	auto result (std::ceil (std::sqrt (size ())));
	return result;
}

bool lightdag::peer_container::empty ()
{
	return size () == 0;
}

bool lightdag::peer_container::not_a_peer (lightdag::endpoint const & endpoint_a)
{
	bool result (false);
	...
	else if (lightdag::reserved_address (endpoint_a))
	{
		result = true;
	}
...
bool lightdag::peer_container::rep_response (lightdag::endpoint const & endpoint_a, lightdag::amount const & weight_a)
{
	auto updated (false);
	std::lock_guard<std::mutex> lock (mutex);
	...
	{
		peers.modify (existing, [weight_a, &updated](lightdag::peer_information & info) {
...
void lightdag::peer_container::rep_request (lightdag::endpoint const & endpoint_a)
{
	std::lock_guard<std::mutex> lock (mutex);
	auto existing (peers.find (endpoint_a));
	if (existing != peers.end ())
	{
		peers.modify (existing, [](lightdag::peer_information & info) {
			info.last_rep_request = std::chrono::steady_clock::now ();
		});
	}
}

bool lightdag::peer_container::reachout (lightdag::endpoint const & endpoint_a)
{
...
bool lightdag::peer_container::insert (lightdag::endpoint const & endpoint_a, unsigned version_a)
{
	auto unknown (false);
	...
	if (!result)
	{
		...
		{
			peers.insert (lightdag::peer_information (endpoint_a, version_a));
			unknown = true;
		}
	}
	if (unknown && !result)
...
bool lightdag::reserved_address (lightdag::endpoint const & endpoint_a)
{
	assert (endpoint_a.address ().is_v6 ());
	...
	else if (bytes.is_loopback () && lightdag::lightdag_network != lightdag::lightdag_networks::lightdag_test_network)
	{
		result = true;
	}
	else if (bytes >= ipv4_loopback_min && bytes <= ipv4_loopback_max && lightdag::lightdag_network != lightdag::lightdag_networks::lightdag_test_network)
	{
		result = true;
	}
	return result;
...
lightdag::peer_information::peer_information (lightdag::endpoint const & endpoint_a, unsigned network_version_a) :
endpoint (endpoint_a),
...
lightdag::peer_information::peer_information (lightdag::endpoint const & endpoint_a, std::chrono::steady_clock::time_point const & last_contact_a, std::chrono::steady_clock::time_point const & last_attempt_a) :
endpoint (endpoint_a),
...
lightdag::peer_container::peer_container (lightdag::endpoint const & self_a) :
self (self_a),
peer_observer ([](lightdag::endpoint const &) {}),
disconnect_observer ([]() {})
{
}

void lightdag::peer_container::contacted (lightdag::endpoint const & endpoint_a, unsigned version_a)
{
	auto endpoint_l (endpoint_a);
	if (endpoint_l.address ().is_v4 ())
	{
...
void lightdag::network::send_buffer (uint8_t const * data_a, size_t size_a, lightdag::endpoint const & endpoint_a, std::function<void(boost::system::error_code const &, size_t)> callback_a)
{
	std::unique_lock<std::mutex> lock (socket_mutex);
	if (node.config.logging.network_packet_logging ())
	{
...
bool lightdag::peer_container::known_peer (lightdag::endpoint const & endpoint_a)
{
	std::lock_guard<std::mutex> lock (mutex);
	auto existing (peers.find (endpoint_a));
...
std::shared_ptr<lightdag::node> lightdag::node::shared ()
{
	return shared_from_this ();
}

lightdag::election::election (MDB_txn * transaction_a, lightdag::node & node_a, std::shared_ptr<lightdag::block> block_a, std::function<void(std::shared_ptr<lightdag::block>, bool)> const & confirmation_action_a) :
...
void lightdag::election::compute_rep_votes (MDB_txn * transaction_a)
{
	node.wallets.foreach_representative (transaction_a, [this, transaction_a](lightdag::public_key const & pub_a, lightdag::raw_key const & prv_a) {
		auto vote (this->node.store.vote_generate (transaction_a, pub_a, prv_a, last_winner));
		this->votes.vote (vote);
	});
}

void lightdag::election::broadcast_winner ()
{
	{
		lightdag::transaction transaction (node.store.environment, nullptr, true);
...
lightdag::uint128_t lightdag::election::quorum_threshold (MDB_txn * transaction_a, lightdag::ledger & ledger_a)
{
	// Threshold over which unanimous voting implies confirmation
	return ledger_a.supply (transaction_a) / 2;
}

lightdag::uint128_t lightdag::election::minimum_threshold (MDB_txn * transaction_a, lightdag::ledger & ledger_a)
{
...
void lightdag::election::confirm_once (MDB_txn * transaction_a)
{
		...
		if (!(*block_l == *last_winner))
		{
			if (exceeded_min_threshold)
			{
				...
				node.background ([node_l, block_l]() {
					node_l->block_processor.process_receive_many (lightdag::block_processor_item (block_l, true));
				});
				last_winner = block_l;
			}
			else
...
bool lightdag::election::have_quorum (MDB_txn * transaction_a)
{
	auto tally_l (node.ledger.tally (transaction_a, votes));
	assert (tally_l.size () > 0);
...
void lightdag::election::confirm_if_quorum (MDB_txn * transaction_a)
{
...
void lightdag::election::confirm_cutoff (MDB_txn * transaction_a)
{
	if (node.config.logging.vote_logging ())
	{
...
bool lightdag::election::vote (std::shared_ptr<lightdag::vote> vote_a)
{
	assert (!lightdag::validate_message (vote_a->account, vote_a->hash (), vote_a->signature));
	...
	if (lightdag::lightdag_network == lightdag::lightdag_networks::lightdag_test_network || weight > supply / 1000) // 0.1% or above
	{
...
void lightdag::active_transactions::announce_votes ()
{
	std::vector<lightdag::block_hash> inactive;
	lightdag::transaction transaction (node.store.environment, nullptr, true);
	std::lock_guard<std::mutex> lock (mutex);

	{
		...
		for (; i != n && announcements < announcements_per_interval; ++i)
		{
			...
			else
			{
				...
				roots.modify (i, [&announcements](lightdag::conflict_info & info_a) {
		...
		for (; i != n; ++i)
		{
			...
			roots.modify (i, [](lightdag::conflict_info & info_a) {
...
void lightdag::active_transactions::stop ()
{
	std::lock_guard<std::mutex> lock (mutex);
	roots.clear ();
}

bool lightdag::active_transactions::start (MDB_txn * transaction_a, std::shared_ptr<lightdag::block> block_a, std::function<void(std::shared_ptr<lightdag::block>, bool)> const & confirmation_action_a)
{
	std::lock_guard<std::mutex> lock (mutex);
	...
	{
		auto election (std::make_shared<lightdag::election> (transaction_a, node, block_a, confirmation_action_a));
		roots.insert (lightdag::conflict_info{ root, election, 0 });
	}
	return existing != roots.end ();
}

// Validate a vote and apply it to the current election if one exists
bool lightdag::active_transactions::vote (std::shared_ptr<lightdag::vote> vote_a)
{
...
bool lightdag::active_transactions::active (lightdag::block const & block_a)
{
	std::lock_guard<std::mutex> lock (mutex);
...
std::deque<std::shared_ptr<lightdag::block>> lightdag::active_transactions::list_blocks ()
{
	std::deque<std::shared_ptr<lightdag::block>> result;
	std::lock_guard<std::mutex> lock (mutex);
...
lightdag::active_transactions::active_transactions (lightdag::node & node_a) :
node (node_a)
{
}

int lightdag::node::store_version ()
{
	lightdag::transaction transaction (store.environment, nullptr, false);
	return store.version_get (transaction);
}

lightdag::thread_runner::thread_runner (boost::asio::io_service & service_a, unsigned service_threads_a)
{
...
lightdag::thread_runner::~thread_runner ()
{
	join ();
}

void lightdag::thread_runner::join ()
{
	for (auto & i : threads)
	{
		if (i.joinable ())
...
void lightdag::add_node_options (boost::program_options::options_description & description_a)
{
	// clang-format off
...
bool lightdag::handle_node_options (boost::program_options::variables_map & vm)
{
	auto result (false);
	boost::filesystem::path data_path = vm.count ("data_path") ? boost::filesystem::path (vm["data_path"].as<std::string> ()) : lightdag::working_path ();
	if (vm.count ("account_create"))
	{
		if (vm.count ("wallet") == 1)
		{
			lightdag::uint256_union wallet_id;
			if (!wallet_id.decode_hex (vm["wallet"].as<std::string> ()))
			{
				...
				if (wallet != nullptr)
				{
					...
					{
						lightdag::transaction transaction (wallet->store.environment, nullptr, true);
	...
	else if (vm.count ("diagnostics"))
	{
		...
		key.data.clear ();
		lightdag::send_block send (0, 0, 0, key, 0, 0);
		std::cout << "Testing key derivation function" << std::endl;
		lightdag::raw_key junk1;
		junk1.data.clear ();
		lightdag::uint256_union junk2 (0);
		lightdag::kdf kdf;
		kdf.phs (junk1, "", junk2);
		std::cout << "Dumping OpenCL information" << std::endl;
		bool error (false);
		lightdag::opencl_environment environment (error);
		if (!error)
		{
		...
		if (vm.count ("wallet") == 1)
		{
			...
			if (!wallet_id.decode_hex (vm["wallet"].as<std::string> ()))
			{
				...
				if (existing != node.node->wallets.items.end ())
				{
					...
					{
						lightdag::transaction transaction (existing->second->store.environment, nullptr, false);
	...
	else if (vm.count ("wallet_list"))
	{
		...
		for (auto i (node.node->wallets.items.begin ()), n (node.node->wallets.items.end ()); i != n; ++i)
		{
			...
			lightdag::transaction transaction (i->second->store.environment, nullptr, false);
			for (auto j (i->second->store.begin (transaction)), m (i->second->store.end ()); j != m; ++j)
			{
				std::cout << lightdag::uint256_union (j->first.uint256 ()).to_account () << '\n';
			}
		}
	}
	else if (vm.count ("wallet_remove"))
	{
		if (vm.count ("wallet") == 1 && vm.count ("account") == 1)
		{
			...
			if (!wallet_id.decode_hex (vm["wallet"].as<std::string> ()))
			{
				...
				{
					lightdag::account account_id;
					if (!account_id.decode_account (vm["account"].as<std::string> ()))
					{
						lightdag::transaction transaction (wallet->second->store.environment, nullptr, true);
	...
	else if (vm.count ("wallet_representative_get"))
	{
		if (vm.count ("wallet") == 1)
		{
			...
			if (!wallet_id.decode_hex (vm["wallet"].as<std::string> ()))
			{
				...
				if (wallet != node.node->wallets.items.end ())
				{
					lightdag::transaction transaction (wallet->second->store.environment, nullptr, false);
	...
	{
		inactive_node node (data_path);
		lightdag::transaction transaction (node.node->store.environment, nullptr, false);
		for (auto i (node.node->store.vote_begin (transaction)), n (node.node->store.vote_end ()); i != n; ++i)
		{
			bool error (false);
			lightdag::bufferstream stream (reinterpret_cast<uint8_t const *> (i->second.data ()), i->second.size ());
			auto vote (std::make_shared<lightdag::vote> (error, stream));
			assert (!error);
...
lightdag::inactive_node::inactive_node (boost::filesystem::path const & path) :
path (path),
...
work (1, nullptr)
{
	...
	logging.init (path);
	node = std::make_shared<lightdag::node> (init, *service, 24000, path, alarm, logging, work);
}

lightdag::inactive_node::~inactive_node ()
{
	node->stop ();
}

lightdag::port_mapping::port_mapping (lightdag::node & node_a) :
node (node_a),
devices (nullptr),
...
void lightdag::port_mapping::start ()
{
	check_mapping_loop ();
}

void lightdag::port_mapping::refresh_devices ()
{
	if (lightdag::lightdag_network != lightdag::lightdag_networks::lightdag_test_network)
	{
		std::lock_guard<std::mutex> lock (mutex);
...
void lightdag::port_mapping::refresh_mapping ()
{
	if (lightdag::lightdag_network != lightdag::lightdag_networks::lightdag_test_network)
	{
		std::lock_guard<std::mutex> lock (mutex);
...
int lightdag::port_mapping::check_mapping ()
{
	int result (3600);
	if (lightdag::lightdag_network != lightdag::lightdag_networks::lightdag_test_network)
	{
...
void lightdag::port_mapping::check_mapping_loop ()
{
...
void lightdag::port_mapping::stop ()
{
	on = false;
	std::lock_guard<std::mutex> lock (mutex);
	for (auto & protocol : protocols)
