...
#include <lightdag/node/testing.hpp>

lightdag::system::system (uint16_t port_a, size_t count_a) :
alarm (service),
work (1, nullptr)
{
	logging.init (lightdag::unique_path ());
	nodes.reserve (count_a);
	for (size_t i (0); i < count_a; ++i)
	{
		lightdag::node_init init;
		lightdag::node_config config (port_a + i, logging);
		auto node (std::make_shared<lightdag::node> (init, service, lightdag::unique_path (), alarm, config, work));
		assert (!init.error ());
		node->start ();
		lightdag::uint256_union wallet;
		lightdag::random_pool.GenerateBlock (wallet.bytes.data (), wallet.bytes.size ());
	...
	auto iterations1 (0);
	while (std::any_of (nodes.begin (), nodes.end (), [](std::shared_ptr<lightdag::node> const & node_a) { return node_a->bootstrap_initiator.in_progress (); }))
	{
		poll ();
		++iterations1;
...
std::shared_ptr<lightdag::wallet> lightdag::system::wallet (size_t index_a)
{
...
lightdag::account lightdag::system::account (MDB_txn * transaction_a, size_t index_a)
{
...
{
public:
	traffic_generator (uint32_t count_a, uint32_t wait_a, std::shared_ptr<lightdag::node> node_a, lightdag::system & system_a) :
	count (count_a),
	wait (wait_a),
	node (node_a),
	system (system_a)
	...
	std::vector<lightdag::account> accounts;
	uint32_t count;
	uint32_t wait;
...
void lightdag::system::generate_usage_traffic (uint32_t count_a, uint32_t wait_a)
{
...
void lightdag::system::generate_usage_traffic (uint32_t count_a, uint32_t wait_a, size_t index_a)
{
	assert (nodes.size () > index_a);
	assert (count_a > 0);
...
void lightdag::system::generate_rollback (lightdag::node & node_a, std::vector<lightdag::account> & accounts_a)
{
	lightdag::transaction transaction (node_a.store.environment, nullptr, true);
...
void lightdag::system::generate_receive (lightdag::node & node_a)
{
	std::shared_ptr<lightdag::block> send_block;
	{
		lightdag::transaction transaction (node_a.store.environment, nullptr, false);
		lightdag::uint256_union random_block;
		random_pool.GenerateBlock (random_block.bytes.data (), sizeof (random_block.bytes));
		auto i (node_a.store.pending_begin (transaction, lightdag::pending_key (random_block, 0)));
		if (i != node_a.store.pending_end ())
		{
			lightdag::pending_key send_hash (i->first);
	...
	{
		auto receive_error (wallet (0)->receive_sync (send_block, lightdag::genesis_account, std::numeric_limits<lightdag::uint128_t>::max ()));
		(void)receive_error;
	}
}

void lightdag::system::generate_activity (lightdag::node & node_a, std::vector<lightdag::account> & accounts_a)
{
...
lightdag::account lightdag::system::get_random_account (std::vector<lightdag::account> & accounts_a)
{
	auto index (random_pool.GenerateWord32 (0, accounts_a.size () - 1));
...
lightdag::uint128_t lightdag::system::get_random_amount (MDB_txn * transaction_a, lightdag::node & node_a, lightdag::account const & account_a)
{
	lightdag::uint128_t balance (node_a.ledger.account_balance (transaction_a, account_a));
	std::string balance_text (balance.convert_to<std::string> ());
	lightdag::uint128_union random_amount;
	random_pool.GenerateBlock (random_amount.bytes.data (), sizeof (random_amount.bytes));
	auto result (((lightdag::uint256_t{ random_amount.number () } * balance) / lightdag::uint256_t{ std::numeric_limits<lightdag::uint128_t>::max () }).convert_to<lightdag::uint128_t> ());
	std::string text (result.convert_to<std::string> ());
	return result;
}

void lightdag::system::generate_send_existing (lightdag::node & node_a, std::vector<lightdag::account> & accounts_a)
{
	...
	lightdag::account source;
	{
		...
		lightdag::transaction transaction (node_a.store.environment, nullptr, false);
		lightdag::store_iterator entry (node_a.store.latest_begin (transaction, account));
		if (entry == node_a.store.latest_end ())
		{
			entry = node_a.store.latest_begin (transaction);
		}
		assert (entry != node_a.store.latest_end ());
		destination = lightdag::account (entry->first.uint256 ());
...
void lightdag::system::generate_change_known (lightdag::node & node_a, std::vector<lightdag::account> & accounts_a)
{
	lightdag::account source (get_random_account (accounts_a));
	if (!node_a.latest (source).is_zero ())
	{
		lightdag::account destination (get_random_account (accounts_a));
		auto change_error (wallet (0)->change_sync (source, destination));
		assert (!change_error);
	}
}

void lightdag::system::generate_change_unknown (lightdag::node & node_a, std::vector<lightdag::account> & accounts_a)
{
	lightdag::account source (get_random_account (accounts_a));
	if (!node_a.latest (source).is_zero ())
	{
		lightdag::keypair key;
		lightdag::account destination (key.pub);
		auto change_error (wallet (0)->change_sync (source, destination));
		assert (!change_error);
	}
}

void lightdag::system::generate_send_new (lightdag::node & node_a, std::vector<lightdag::account> & accounts_a)
{
...
void lightdag::system::generate_mass_activity (uint32_t count_a, lightdag::node & node_a)
{
	std::vector<lightdag::account> accounts;
	wallet (0)->insert_adhoc (lightdag::test_genesis_key.prv);
	accounts.push_back (lightdag::test_genesis_key.pub);
...
lightdag::landing_store::landing_store ()
{
}

lightdag::landing_store::landing_store (lightdag::account const & source_a, lightdag::account const & destination_a, uint64_t start_a, uint64_t last_a) :
source (source_a),
...
lightdag::landing_store::landing_store (bool & error_a, std::istream & stream_a)
{
	error_a = deserialize (stream_a);
}

bool lightdag::landing_store::deserialize (std::istream & stream_a)
{
	bool result;
	try
	{
...
void lightdag::landing_store::serialize (std::ostream & stream_a) const
{
	boost::property_tree::ptree tree;
...
bool lightdag::landing_store::operator== (lightdag::landing_store const & other_a) const
{
	return source == other_a.source && destination == other_a.destination && start == other_a.start && last == other_a.last;
}

lightdag::landing::landing (lightdag::node & node_a, std::shared_ptr<lightdag::wallet> wallet_a, lightdag::landing_store & store_a, boost::filesystem::path const & path_a) :
path (path_a),
store (store_a),
wallet (wallet_a),
node (node_a)
...
void lightdag::landing::write_store ()
{
...
lightdag::uint128_t lightdag::landing::distribution_amount (uint64_t interval)
{
	...
	if (interval < intervals_per_period * 1)
	{
		...
		// 2^128 / 2^1 / (2^25 / 2^10)
		result = lightdag::uint128_t (1) << (127 - (25 - interval_exponent)); // 50%
	}
	else if (interval < intervals_per_period * 2)
	{
		result = lightdag::uint128_t (1) << (126 - (25 - interval_exponent)); // 25%
	}
	else if (interval < intervals_per_period * 3)
	{
		result = lightdag::uint128_t (1) << (125 - (25 - interval_exponent)); // 13%
	}
	else if (interval < intervals_per_period * 4)
	{
		result = lightdag::uint128_t (1) << (124 - (25 - interval_exponent)); // 6.3%
	}
	else if (interval < intervals_per_period * 5)
	{
		result = lightdag::uint128_t (1) << (123 - (25 - interval_exponent)); // 3.1%
	}
	else if (interval < intervals_per_period * 6)
	{
		result = lightdag::uint128_t (1) << (122 - (25 - interval_exponent)); // 1.6%
	}
	else if (interval < intervals_per_period * 7)
	{
		result = lightdag::uint128_t (1) << (121 - (25 - interval_exponent)); // 0.8%
	}
...
void lightdag::landing::distribute_one ()
{
	auto now (lightdag::seconds_since_epoch ());
	lightdag::block_hash last (1);
...
void lightdag::landing::distribute_ongoing ()
{
	distribute_one ();
...
std::chrono::seconds constexpr lightdag::landing::distribution_interval;
std::chrono::seconds constexpr lightdag::landing::sleep_seconds;
